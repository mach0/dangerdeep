...
class texture
{
  ...
  protected:
    ...
    GLenum dimension;
    mapping_mode
        mapping; // how GL draws the texture (GL_NEAREST, GL_LINEAR, etc.)
    clamping_mode
  ...
  public:
    ...
    void draw_hm(int x, int y, const colorf& col = colorf(1, 1, 1, 1))
        const; // horizontally mirrored
    void draw(const vector2i& p, const colorf& col = colorf(1, 1, 1, 1)) const
    {
        draw(p.x, p.y, col);
    }
    void
    draw_hm(const vector2i& p, const colorf& col = colorf(1, 1, 1, 1)) const
    {
        draw_hm(p.x, p.y, col);
    }
