...
template <typename NodeData, typename EdgeData> class graph
{
  public:
    ...
    void for_all_edges(
        const std::function<void(const vector2u&, const EdgeData&)>& func)
    {
    ...
    /// Set node data
    void set_node_data(unsigned n, NodeData&& d)
    {
        node_data[n] = std::move(d);
    }

    /// Set edge data
    void set_edge_data(unsigned n, EdgeData&& d)
    {
        edge_data[n] = std::move(d);
    }
  ...
  protected:
    ...
    std::vector<unsigned>
        neighbor_indices; //!< For every node an index in neighbors
