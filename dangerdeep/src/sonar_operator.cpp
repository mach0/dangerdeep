...
/* how the sonar operator works:
   He turns the apparatus at constant speed around the compass, with 2 or 3
   degrees per simulation step, total speed of 6 degrees per second or so. When
   he detects a transition from growing strength to falling strengh of the
   signal, he turns the apparatus in opposite direction with slower speed (0.5-1
   degree per simulation step) and detects the angle where the signal reaches
   its maximum (upper limit), turns back until signal gets weaker (lower limit),
   then reports the mid between the lower and upper limit as contact, turns the
   apparatus back to upper limit and starts over. If a signal is very strong and
   thus close, he switches to tracking mode. In tracking mode he turns the
   apparatus around +- 30 degrees around the strongest signal and redects its
   position with the mentioned algorithm until the signal gets weaker. Or he
   turns the apparatus away from current peak angle until signal gets weaker,
   then back. Always searching for the limit of the signal, following a
   direction only if signal gets stronger along the direction. Direction is
   constantly changed between clockwise and counter-clockwise in that case.
   fixme - when does the operator choose the frequency band switch? This must
   depend on how far/strong a signal is... So we have several states (circular
   mostly) 1) Just turning and listening to strengths -> detect signal with
   growing strength when new strength > old strength, then to 2) 2) Following
   signal increase -> detect weakening of signal, then to 3) 3) Detect upper
   limit by finer turning in opposite direction, when new signal is <= current
      signal. Equal -> 4), less -> 5)
   4) Detect lower limit of signal by finer turning, until new signal is weaker
   than current, then 5) 5) report mid of lower and upper as new contact, turn
   current angle to upper with normal speed then back to 1) 5) is same as one...
*/
void sonar_operator::simulate(game& gm, double delta_t)
{
    ...
    // signals are reported, although you can see more definitive peaks in the
    // higher frequencies.
    ...
    switch (state)
    {
        ...
        case find_growing_signal:
            ...
            {
                advance_angle_and_erase_old_contacts(
                    (turn_speed_fast - sub_turn_velocity) * simulation_step,
                    sub_heading);
            }
            current_signal_strength = signal.first;
            break;
                ...
                else
                {
                    ...
                    if (find_peak_try > 1)
                    {
                        ...
                        add_contact(
                            ...
                            contact(signal.first, sc));
                        state = find_growing_signal;
                        current_angle += angle(
                            (turn_speed_fast - sub_turn_velocity)
                            * simulation_step);
                    }
                }
            }
    ...
    angle sub_heading)
{
    ...
    // do not set from next_angle here, because that contains the sub's heading
    // as additional factor but add addang to current_angle. In short, don't mix
    // relative and absolute angles! be careful...
    current_angle += angle(addang);
}

void sonar_operator::add_contact(angle absang, const contact& ct)
{
    // erase any values within +-n degree range around the new contact, could be
    // same contact when sub is turning
