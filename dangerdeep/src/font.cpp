...
font::font(const string& basefilename, unsigned char_spacing)
{
    ...
    {
        static const char* vs = "varying vec2 texcoord;\n"
                                "void main(){\n"
                                "texcoord = gl_MultiTexCoord0.xy;\n"
                                "gl_Position = ftransform();\n"
                                "}\n";
        static const char* fs = "uniform sampler2D tex;\n"
    ...
    character_texture = std::make_unique<texture>(
        basefilename + ".png", texture::LINEAR, texture::CLAMP);
    ...
    auto codex  = unsigned('x');
    blank_width = (codex >= first_char && codex <= last_char)
                      ? characters[codex - first_char].width
                      : 8;
    height      = base_height * 3 / 2;
    base_height =
        base_height * 7 / 6; // tiny trick to use the space a bit better.
}
    ...
    unsigned maxheight) const
{
    ...
    while (true)
    {
        ...
        if (textlen > 0 && textptr == 0)
        { // space is not enough to wrap first word. so disable wrapping
            w = 0xffffffff;
            continue;
        }
        if (breaktext)
        ...
        {
            unsigned tw =
                get_size(text.substr(oldtextptr, textptr - oldtextptr)).x;
            if (currwidth + tw > w)
            {
                y += (lineheight == 0) ? get_height() : lineheight;
                currwidth = 0;
...
unsigned font::read_character(const std::string& text, unsigned cp)
{
    ...
    // 0xE0 | (upper 4 bits of c), 0x80 | (middle 6 bits of c), 0x80 | (lower 6
    // bits of c) Characters from 0x010000-0x10FFFF are encoded as four bytes:
