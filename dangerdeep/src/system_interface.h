...
class system_interface : public singleton<system_interface>
{
  public:
    ...
    struct parameters
    {
        ...
        vector2i resolution2d{
            1024,
            768}; ///< X/Y-Resolution of 2d mode to use (virtual resolution)
        ...
        double far_z{
            30000.0}; // not needed with new gpu interface, remove later
        parameters() { }
    };

    /// used to handle out-of-order quit events, do NOT inherit from
    /// std::exception!
    ...
    /// Finish drawing of frame in window, will trigger fetching of input
    /// events, returns true if program should quit!
    bool finish_frame();

    /// Return global time stamp in milliseconds (inactive process time not
    /// counted!)
    uint32_t millisec();
    ...
    vector2f translate_2D_coordinates(const vector2i& c) const
    {
        return vector2f(
            ...
            scale_pseudo_2D.y * c.y + offset_pseudo_2D.y);
    }
  ...
  private:
    ...
    uint32_t time_passed_while_sleeping{
        0}; ///< Total time in milliseconds that passed while application slept
    ...
    vector2i mouse_position_2d; ///< Last known mouse position (pseudo 2d
                                ///< coordinates)
    std::string screenshot_dir; ///< Directory for screenshots
    std::vector<vector2i>
        available_resolutions; ///< List of available screen resolutions
    std::vector<std::weak_ptr<input_event_handler>>
        input_event_handlers; ///< stack of input event handlers
