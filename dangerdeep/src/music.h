...
class music : public singleton<class music>, public ::thread
{
 ...
 protected:
	...
	class command_play_sfx : public message
	{
	 ...
	 public:
		command_play_sfx(music& my_music_, std::string  category_, vector3  listener_, angle listener_dir_, vector3  noise_pos_) : my_music(my_music_), category(std::move(category_)), listener(std::move(listener_)), listener_dir(std::move(listener_dir_)), noise_pos(std::move(noise_pos_)) {}
	};
