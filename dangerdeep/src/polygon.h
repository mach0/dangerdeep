...
public:
	...
	polygon_t(const vector3t<D>& a, const vector3t<D>& b, const vector3t<D>& c) : points{a, b, c} {}
	...
	template<typename E> polygon_t(const polygon_t<E>& other)
	...
	unsigned next_index(unsigned i) const { ++i; if (i >= nr_of_points()) i = 0; return i; }
	/// Get previous index from this
	unsigned prev_index(unsigned i) const { if (i == 0) i = nr_of_points(); --i; return i; }
	...
	struct plane_type_generic
	{
		...
		vector3t<D> intersection(const vector3t<D>& a, const vector3t<D>& b) { return pln.intersection(a, b); }
	};
	...
	template<axis A> struct plane_type_axis
	{
		...
		D axis_value;
		D distance(const vector3t<D>& p) { return plane_distance<D, A>(p, axis_value); }
		vector3t<D> intersection(const vector3t<D>& a, const vector3t<D>& b) { return plane_intersection<D, A>(a, b, axis_value); }
	};

	/// Clip polygon with plane, template arguments to return only front side or also back side, to use special plane
	/// and to get also cut points. This is some template overkill, but otherwise we would have the same function
	/// in many variants with much code duplication.
	template<typename result_type, typename cut_point_type, typename plane_type> bool clip_generic(result_type& rt, cut_point_type& cpt, plane_type& pt) const
	{
		// construct one or two resulting polygons.
		// iterate over points (edges), if a point is left or right, assign to result
		// polygon respectivly. If point is on different side than previous point,
		// compute intersection and insert it into both results.
		// Avoid to generate very short edges, so if points are very close to the plane
		// use them as intersection points.
		// However points on the plane should not be counted for side switching,
		// a polygon touching the plane is just kept as it is.
		// If first point is on plane we have to go back until we know which side it belongs to.
		if (empty()) {
			return false;
		}
		...
		int last_point_side = (last_point_distance > epsilon) ? 1 : (last_point_distance < -epsilon ? -1 : 0);
		...
		if (current_side == 0) {
			...
			for (unsigned i = last_point_index; i > 0 && current_side == 0; --i) {
				...
				current_side = (this_point_distance > epsilon) ? 1 : (this_point_distance < -epsilon ? -1 : 0);
			}
			if (current_side == 0) {
		...
		for (unsigned i = 0; i < nr_of_points(); ++i) {
			...
			int this_point_side = (this_point_distance > epsilon) ? 1 : (this_point_distance < -epsilon ? -1 : 0);
			if (this_point_side > 0) {
				if (last_point_side < 0) {
					// compute intersection
					vector3t<D> its = pt.intersection(points[last_point_index], points[i]);
				...
				// last point was front or on plane or intersection, just add to front
				...
				// last point was back or on plane or intersection, just add to back
	...
	/// Clip with axis aligned plane with frontside/backside result
	template<axis a> std::pair<polygon_t, polygon_t> clip(plane_type_axis<a> pta) const
	{
		clip_result_front_and_back crfb;
		clip_point_no_result cpnr;
		clip_generic(crfb, cpnr, pta);
	...
	/// Check if a point is inside the polygon from a given direction. Here we assume z axis as direction.
	/// determine number of intersections of polygon with one axis going out of the point (e.g. X axis),
	/// even number is outside
	bool is_inside(const vector2t<D>& point) const
	{
		...
		for (unsigned i = 0; i < unsigned(points.size()); ++i) {
			...
			if (last_point_above != point_above) {
				// edge intersects X axis, determine where, if its right of the point
				D t = delta_y / (delta_y - last_delta_y);
				D x = points[last_index].x * t + points[i].x * (D(1) - t);
				num_intersections += (x > point.x) ? 1 : 0;
				//std::cout<<"t="<<t<<" x="<<x<<" numi "<<num_intersections<<"\n";//fixme seems ok, but needs more testing
			}
		}
		return (num_intersections & 1);
	}
	/// Compute unnormalized normal of a special polygon, a triangle
	static vector3t<D> compute_normal(const vector3t<D>& a, const vector3t<D>& b, const vector3t<D>& c)
	{
		return (b - a).cross(c - a);
	}
	/// Clip a vector of polygons by one plane and fetch results to two other vectors
	template<axis a> static void clip(const std::vector<polygon_t>& src, std::vector<polygon_t>& front,
...
public:
	std::vector<vector3t<D>> points;	///< The points in 3-space forming the polygon
};
...
{
	// check special cases
	#define A(x,y) p.add_point(vector3f(float(x),float(y),0.0f)); // initializer list ctor is better
	std::cout << "case 0\n";
