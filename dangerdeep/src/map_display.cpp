    ...
    const vector2& offset, const sea_object* so, color c) const
{
    ...
    primitives::line(
        ...
        vector2f(p.x + d.x * l, p.y - d.y * l), c)
        .render();
}
...
void map_display::draw_sound_contact(
    class game& gm, const sea_object* player, double max_view_dist,
    const vector2& offset) const
{
    ...
    for (const auto& it : obj)
    {
        ...
        primitives::line(
            ...
            vector2f(512 + pos.x + ldir.x, 384 - pos.y - ldir.y), col)
            .render();
    }
}

void map_display::draw_sound_contact(
    game& gm, const submarine* player, const vector2& offset) const
{
...
void map_display::draw_visual_contacts(
    class game& gm, const sea_object* player, const vector2& offset) const
{
...
void map_display::draw_square_mark(
    class game& gm, const vector2& mark_pos, const vector2& offset,
    const color& c) const
{
    ...
    primitives::rectangle(
        vector2f(512 - 4 + x, 384 - 4 - y), vector2f(512 + 4 + x, 384 + 4 - y),
        c)
        .render();
}
    ...
    const color& c) const
{
    ...
    int y     = int(round(p.y));
    primitives::rectangle(
        vector2f(512 - 8 + x, 384 - 8 - y), vector2f(512 + 8 + x, 384 + 8 - y),
        c)
        .render();
    ...
    if (ge)
    {
        ...
        edit_panel->add_child(
            std::make_unique<widget_caller_button<map_display&, game_editor&>>(
                0, 0, 128, 32, texts::get(224), nullptr,
                [](auto& md, auto& gme) { md.edit_add_obj(gme); }, *this, gme));
        edit_btn_del = &edit_panel->add_child(
            std::make_unique<widget_caller_button<map_display&, game_editor&>>(
                128, 0, 128, 32, texts::get(225), nullptr,
                [](auto& md, auto& gme) { md.edit_del_obj(gme); }, *this, gme));
        edit_btn_chgmot = &edit_panel->add_child(
            std::make_unique<widget_caller_button<map_display&, game_editor&>>(
                256, 0, 128, 32, texts::get(226), nullptr,
                [](auto& md, auto& gme) { md.edit_change_motion(gme); }, *this,
                gme));
        edit_btn_copy = &edit_panel->add_child(
            std::make_unique<widget_caller_button<map_display&, game_editor&>>(
                384, 0, 128, 32, texts::get(227), nullptr,
                [](auto& md, auto& gme) { md.edit_copy_obj(gme); }, *this,
                gme));
        edit_btn_cvmenu = &edit_panel->add_child(
            std::make_unique<widget_caller_button<map_display&, game_editor&>>(
                512, 0, 128, 32, texts::get(228), nullptr,
                [](auto& md, auto& gme) { md.edit_convoy_menu(gme); }, *this,
                gme));
        edit_panel->add_child(
            std::make_unique<widget_caller_button<map_display&, game_editor&>>(
                640, 0, 128, 32, texts::get(229), nullptr,
                [](auto& md, auto& gme) { md.edit_time(gme); }, *this, gme));
        edit_panel->add_child(
            std::make_unique<widget_caller_button<map_display&, game_editor&>>(
                768, 0, 128, 32, texts::get(233), nullptr,
                [](auto& md, auto& gme) { md.edit_description(gme); }, *this,
                gme));
        edit_panel->add_child(
            std::make_unique<widget_caller_button<map_display&, game_editor&>>(
                896, 0, 128, 32, texts::get(230), nullptr,
                [](auto& md, auto& gme) { md.edit_help(gme); }, *this, gme));
        ...
        edit_panel_add->add_child(
            std::make_unique<widget_caller_button<widget&>>(
                20, 768 - 3 * 32 - 8, 512 - 20, 32, texts::get(224), nullptr,
                [](auto& w) { w.close(EPFG_SHIPADDED); }, *edit_panel_add));
        edit_panel_add->add_child(
            std::make_unique<widget_caller_button<widget&>>(
                512, 768 - 3 * 32 - 8, 512 - 20, 32, texts::get(117), nullptr,
                [](auto& w) { w.close(EPFG_CANCEL); }, *edit_panel_add));
        ...
        edit_speed =
            &edit_panel_chgmot->add_child(std::make_unique<widget_slider>(
                20, 220, 1024 - 40, 80, texts::get(4), 0 /*minspeed*/,
                34 /*maxspeed*/, 0, 1));
        edit_throttle =
            &edit_panel_chgmot->add_child(std::make_unique<widget_slider>(
                20, 330, 1024 - 40, 80, texts::get(232), 0 /*minspeed*/,
                34 /*maxspeed*/, 0, 1));
        edit_panel_chgmot->add_child(
            std::make_unique<widget_caller_button<widget&>>(
                20, 768 - 3 * 32 - 8, 512 - 20, 32, texts::get(226), nullptr,
                [](auto& w) { w.close(EPFG_CHANGEMOTION); },
                *edit_panel_chgmot));
        edit_panel_chgmot->add_child(
            std::make_unique<widget_caller_button<widget&>>(
                512, 768 - 3 * 32 - 8, 512 - 20, 32, texts::get(117), nullptr,
                [](auto& w) { w.close(EPFG_CANCEL); }, *edit_panel_chgmot));
        ...
        edit_panel_help->add_child(std::make_unique<widget_text>(
            20, 32, 1024 - 2 * 20, 768 - 2 * 32 - 2 * 32 - 8, texts::get(231),
            nullptr, true));
        edit_panel_help->add_child(
            std::make_unique<widget_caller_button<widget&>>(
                20, 768 - 3 * 32 - 8, 1024 - 20, 32, texts::get(105), nullptr,
                [](auto& w) { w.close(EPFG_CANCEL); }, *edit_panel_help));
        ...
        edit_panel_time->add_child(
            std::make_unique<widget_caller_button<widget&>>(
                20, 768 - 3 * 32 - 8, 512 - 20, 32, texts::get(229), nullptr,
                [](auto& w) { w.close(EPFG_CHANGETIME); }, *edit_panel_time));
        edit_panel_time->add_child(
            std::make_unique<widget_caller_button<widget&>>(
                512, 768 - 3 * 32 - 8, 512 - 20, 32, texts::get(117), nullptr,
                [](auto& w) { w.close(EPFG_CANCEL); }, *edit_panel_time));
        edit_timeyear =
        ...
        edit_panel_convoy->add_child(
            std::make_unique<widget_caller_button<widget&>>(
                20 + 0 * (1024 - 40) / 5, 768 - 3 * 32 - 8, (1024 - 40) / 5, 32,
                texts::get(240), nullptr,
                [](auto& w) { w.close(EPFG_ADDSELTOCV); }, *edit_panel_convoy));
        edit_panel_convoy->add_child(
            std::make_unique<widget_caller_button<widget&>>(
                20 + 1 * (1024 - 40) / 5, 768 - 3 * 32 - 8, (1024 - 40) / 5, 32,
                texts::get(241), nullptr,
                [](auto& w) { w.close(EPFG_MAKENEWCV); }, *edit_panel_convoy));
        edit_panel_convoy->add_child(
            std::make_unique<widget_caller_button<widget&>>(
                20 + 2 * (1024 - 40) / 5, 768 - 3 * 32 - 8, (1024 - 40) / 5, 32,
                texts::get(242), nullptr,
                [](auto& w) { w.close(EPFG_DELETECV); }, *edit_panel_convoy));
        edit_panel_convoy->add_child(
            std::make_unique<widget_caller_button<widget&>>(
                20 + 3 * (1024 - 40) / 5, 768 - 3 * 32 - 8, (1024 - 40) / 5, 32,
                texts::get(243), nullptr,
                [](auto& w) { w.close(EPFG_EDITROUTECV); },
                *edit_panel_convoy));
        edit_panel_convoy->add_child(
            std::make_unique<widget_caller_button<widget&>>(
                20 + 4 * (1024 - 40) / 5, 768 - 3 * 32 - 8, (1024 - 40) / 5, 32,
                texts::get(117), nullptr, [](auto& w) { w.close(EPFG_CANCEL); },
                *edit_panel_convoy));
...
void map_display::display() const
{
    ...
    primitives::circle(
        ...
        max_view_dist * mapzoom, colorf(1, 0, 0))
        .render();
    ...
    if (sub_player && sub_player->is_submerged())
    {
            ...
            && sub_player->is_scope_up())
        {
            ...
            {
                draw_square_mark(
                    gm, gm.get_object(target).get_pos().xy(), -offset,
                    color(255, 0, 0));
            }
        }
    }
...
//	}
    ...
    if (gm.is_editor())
    {
        ...
        else
        {
            ...
            {
                draw_square_mark(
                    gm, gm.get_object(it).get_pos().xy(), -offset,
                    color(255, 0, 64));
            }
        }
        edit_panel->draw();
