...
void matrix_invert(D* values)
{
    ...
    { // columns of L
        ...
        { // rows of L
            ...
            { // columns of R
                values[j * size + k] -=
                    values[j * size + i] * values[i * size + k];
            }
        }
    }
...
class matrixt
{
  ...
  public:
    ...
    static matrixt<D, size> one()
    {
        matrixt<D, size> r;
        for (unsigned i = 0; i < size; ++i)
            r.values[i + i * size] = D(1.0);
        return r;
    }
    ...
    matrixt<D, size> operator*(const matrixt<D, size>& other) const
    {
        ...
        { // columns of "other"
            ...
            { // rows of "this"
                D tmp = D(0.0);
                for (unsigned k = 0; k < size;
                     ++k) // columns of "this"/rows of "other"
    ...
    {
        return values[col + row * size];
    }
    D& elem_at(unsigned col, unsigned row)
    {
        return values.at(col + row * size);
    }
    const D& elem_at(unsigned col, unsigned row) const
    {
        return values.at(col + row * size);
    }
    ...
    vector4t<D> operator*(const vector4t<D>& v) const
    {
        ...
        { // rows of "this"
            r[j] = values[j * 4 + 0] * v.x + values[j * 4 + 1] * v.y
                   + values[j * 4 + 2] * v.z + values[j * 4 + 3] * v.w;
        }
