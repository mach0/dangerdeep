// RCHashArray.hpp                                           Copyright (C) 2006
...
class CRCHashArray
{
  ...
  protected:
    ...
    struct SHashArrayNode
    {
        ...
        CRenderingContext*
            pRenderingContext; //!<           The rendering context.
    };
  ...
  public:
    ...
    CRenderingContext*
    GetRenderingContext(HRCKEY hRCKey = ::wglGetCurrentContext()); //  return rc
    CRenderingContext* PrepareRenderingContext(
        HRCKEY hRCKey = ::wglGetCurrentContext()); //  create rc

#elif (defined(__APPLE__) && defined(__GNUC__)) || defined(__MACOSX__)

    CRenderingContext*
    GetRenderingContext(HRCKEY hRCKey = ::aglGetCurrentContext()); //  return rc
    CRenderingContext* PrepareRenderingContext(
        HRCKEY hRCKey = ::aglGetCurrentContext()); //  create rc

#else // _WIN32

    CRenderingContext*
    GetRenderingContext(HRCKEY hRCKey = ::glXGetCurrentContext()); //  return rc
    CRenderingContext* PrepareRenderingContext(
        HRCKEY hRCKey = ::glXGetCurrentContext()); //  create rc
  ...
  protected:
    ...
    SHashArrayNode*
        m_pHashArray[HASH_SIZE]; //!<                  The hash array.
};
...
//!	Return the rendering context for a given key (or create a new one if
//! necessary).
