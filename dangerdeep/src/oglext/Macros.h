...
#define WRAPPER00V(NAME) \
    gl##NAME() { WRAPPER_CALL_V(NAME)(); }

#define WRAPPER01V(NAME, T1) \
    gl##NAME(T1 v1) { WRAPPER_CALL_V(NAME)(v1); }

#define WRAPPER02V(NAME, T1, T2) \
    gl##NAME(T1 v1, T2 v2) { WRAPPER_CALL_V(NAME)(v1, v2); }

#define WRAPPER03V(NAME, T1, T2, T3) \
    gl##NAME(T1 v1, T2 v2, T3 v3) { WRAPPER_CALL_V(NAME)(v1, v2, v3); }
...
#define WRAPPER00R(NAME) \
    gl##NAME() { WRAPPER_CALL_R(NAME)(); }

#define WRAPPER01R(NAME, T1) \
    gl##NAME(T1 v1) { WRAPPER_CALL_R(NAME)(v1); }

#define WRAPPER02R(NAME, T1, T2) \
    gl##NAME(T1 v1, T2 v2) { WRAPPER_CALL_R(NAME)(v1, v2); }

#define WRAPPER03R(NAME, T1, T2, T3) \
    gl##NAME(T1 v1, T2 v2, T3 v3) { WRAPPER_CALL_R(NAME)(v1, v2, v3); }
...
#define GET_PROC_ADDRESS(NAME)                                  \
    *((void**) &m_p##NAME) = GetProcAddress(#NAME);             \
    if (m_p##NAME == NULL)                                      \
    {                                                           \
        ::OutputDebugString("OglExt warning: Function \"" #NAME \
                            "\" not linkable!\n");              \
        bReturn = false;                                        \
    }
...
#define INIT_EXTENSION(NAME, FUNC)                                 \
    ...
    {                                                              \
        ...
        if (!br)                                                   \
        {                                                          \
            ::OutputDebugString("==> OglExt warning: " NAME        \
                                " supported but not linkable!\n"); \
        }                                                          \
    }
...
#define INIT_GLVERSION(MAJOR, MINOR, RELEASE, FUNC)                          \
    ...
    {                                                                        \
        ...
        if (!br)                                                             \
        {                                                                    \
            ::OutputDebugString("OglExt warning: Version " #MAJOR "." #MINOR \
                                "." #RELEASE                                 \
                                " supported but not linkable!\n");           \
        }                                                                    \
    }
