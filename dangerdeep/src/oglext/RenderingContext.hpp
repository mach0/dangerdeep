// RenderingContext.hpp                                      Copyright (C) 2006
...
class CRenderingContext
{
  ...
  public:
    ...
    static void* GetProcAddress(
        char const* szFunction); //    cross-platform GetProcAddress()
...
inline void CRenderingContext::InitExtensionString()
{
    ...
    // 2: traverse through the extension string and build up the extension name
    // list...
...
inline bool
CRenderingContext::IsExtensionSupported(char const* szExtensions) const
{
    ...
    do
    {
        ...
        while (pExtensionName)
        {
            ...
            // 1.1.1: search for the end of one extension name, or a difference
            // in names...

            char const* szWant = szExtensions;
            char const* szThis = pExtensionName->szExtension;

            while ((*szThis != '\0') && (*szThis != ' ')
                   && (*szWant == *szThis))
            {
            ...
            // 1.1.2: if we reached the end of the extension name, we continue
        ...
        // 1.2: reached end of the supported extensions => specified extension
        // is not supported...
