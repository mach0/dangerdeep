...
void tone_reproductor::set_display_adaptation_luminance(float _Lda)
{
    ...
    term2                  = float(
        mypow10((beta_wa - beta_da) / alpha_da) / (constant::PI * 0.0001f));
}
...
void tone_reproductor::xyY_to_RGB(float* color)
{
    ...
    if (log10Y < 0.6)
    {
        ...
        color[0] = (1.f - s) * 0.25f
                   + s * color[0]; // Add scotopic + photopic components
        color[1] = (1.f - s) * 0.25f
                   + s * color[1]; // Add scotopic + photopic components

        // Take into account the scotopic luminance approximated by V [3] [4]
        float V = color[2]
                  * (1.33f
                         * (1.f + color[1] / color[0]
                            + color[0] * (1.f - color[0] - color[1]))
                     - 1.68f);
