...
template <class T> class morton_bivector
{
  ...
  public:
    ...
    {
        resize(std::max(
            pow(2, ceil(log2(bv.datasize.x))),
            pow(2, ceil(log2(bv.datasize.y)))));
    ...
    morton_bivector(const long& sz, const T& v = T()) :
        datasize(sz), data(sz * sz, v)
    {
        if (log2(sz) != (int) log2(sz))
            throw "morton_bivector::morton_bivector() wrong datasize: size is "
                  "not power of 2";
        morton_x.resize(sz);
        morton_y.resize(sz);
    ...
    morton_bivector<T>&
    add_shifted(const bivector<T>& other, const vector2i& offset);

    morton_bivector<T>&
    insert(const bivector<T>& other, const vector2i& offset);
...
morton_bivector<T>::sub_area(const vector2i& offset, const long& sz) const
{
    if (offset.y + sz > datasize)
        THROW(error, "morton_bivector::sub_area, offset.y invalid");
    if (offset.x + sz > datasize)
        THROW(error, "morton_bivector::sub_area, offset.x invalid");
...
morton_bivector<T> morton_bivector<T>::shifted(const vector2i& offset) const
{
    ...
    bivector_FOREACH_XYZ(
        result.at(
            (x + offset.x) & (datasize - 1), (y + offset.y) & (datasize - 1)) =
    ...
    */
    ...
    if (wrap)
    {
        ...
        for (int y = 0; y < datasize; ++y)
        {
            ...
            result.at(2 * datasize - 1, 2 * y) =
                T((at(datasize - 1, y) + at(0, y)) * 0.5);
        }
    }
    else
    {
    ...
    if (wrap)
    {
        ...
        for (int x = 0; x < resultsize; ++x)
        {
            result.at(x, resultsize - 1) =
                T((result.at(x, resultsize - 2) + result.at(x, 0)) * 0.5);
        }
    }
    return result;
}
    ...
    */
    ...
    if (!force_even_size)
    {
        ...
        if (datasize & 1)
        {
            for (int y = 0; y < newsize; ++y)
                result.at(newsize, y) =
                    T((at(datasize - 1, 2 * y) + at(datasize - 1, 2 * y + 1))
                      * 0.5);
        }
        if (datasize & 1)
        {
            for (int x = 0; x < newsize; ++x)
                result.at(x, newsize) =
                    T((at(2 * x, datasize - 1) + at(2 * x + 1, datasize - 1))
                      * 0.5);
        }
...
morton_bivector<T> morton_bivector<T>::smooth_upsampled(bool wrap) const
{
    ...
    */
    if (datasize < 3)
        THROW(error, "morton_bivector::smooth_upsampled base size invalid");
    long resultsize = wrap ? datasize * 2 : datasize * 2 - 1;
    morton_bivector<T> result(resultsize);
    // copy values that are kept and interpolate missing values on even rows
    for (int y = 0; y < datasize; ++y)
    {
        ...
        for (int x = 1; x < datasize - 2; ++x)
        {
            result.at(2 * x, 2 * y) = at(x, y);
            result.at(2 * x + 1, 2 * y) =
                T(at(x - 1, y) * c1[0] + at(x, y) * c1[1] + at(x + 1, y) * c1[2]
                  + at(x + 2, y) * c1[3]);
        }
    ...
    if (wrap)
    {
        ...
        {
            result.at(1, 2 * y) =
                T(at(datasize - 1, y) * c1[0] + at(0, y) * c1[1]
                  + at(1, y) * c1[2] + at(2, y) * c1[3]);
            result.at(2 * datasize - 3, 2 * y) =
                T(at(datasize - 3, y) * c1[0] + at(datasize - 2, y) * c1[1]
                  + at(datasize - 1, y) * c1[2] + at(0, y) * c1[3]);
            result.at(2 * datasize - 1, 2 * y) =
                T(at(datasize - 2, y) * c1[0] + at(datasize - 1, y) * c1[1]
    ...
    else
    {
        for (int y = 0; y < datasize; ++y)
        {
            ...
            result.at(2 * datasize - 3, 2 * y) =
                T(at(datasize - 3, y) * c1[0] + at(datasize - 2, y) * c1[1]
                  + at(datasize - 1, y) * c1[2] + at(datasize - 1, y) * c1[3]);
        }
    }
    // interpolate missing values on odd rows
    for (int y = 1; y < datasize - 2; ++y)
    {
        ...
        {
            result.at(x, 2 * y + 1) =
                T(result.at(x, 2 * y - 2) * c1[0] + result.at(x, 2 * y) * c1[1]
    ...
    if (wrap)
    {
        ...
        {
            result.at(x, 1) = T(
                result.at(x, 2 * datasize - 2) * c1[0] + result.at(x, 0) * c1[1]
                + result.at(x, 2) * c1[2] + result.at(x, 4) * c1[3]);
            result.at(x, 2 * datasize - 3) =
                T(result.at(x, 2 * datasize - 6) * c1[0]
                  + result.at(x, 2 * datasize - 4) * c1[1]
                  + result.at(x, 2 * datasize - 2) * c1[2]
                  + result.at(x, 0) * c1[3]);
            result.at(x, 2 * datasize - 1) =
                T(result.at(x, 2 * datasize - 4) * c1[0]
...
{
    bivector_FOREACH_XY(
        at(x, y) += other.at(x & (other.datasize - 1), y & (other.datasize - 1))
                    * scal;) return *this;
}
