...
class mesh
{
  public:
    ...
    class triangle_index
    {
      public:
        ...
        bool operator==(const triangle_index& t) const
        {
    ...
    class vertex_index
    {
      public:
        ...
        bool operator==(const vertex_index& v) const
        {
    ...
    mesh(
        ...
        std::vector<std::array<vertex_index, 3>>&& indices,
    ...
    auto vertex(triangle_index ti, unsigned ci) const
    {
        return indices[ti.get_index()][ci];
    }

    /// return position of vertex
    const auto& position(vertex_index vi) const
    {
        return positions[vi.get_index()];
    }

    /// return normal of vertex
    const auto& normal(vertex_index vi) const
    {
        return normals[vi.get_index()];
    }

    /// return texcoord of vertex
    const auto& texcoord(vertex_index vi) const
    {
        return texcoords[vi.get_index()];
    }

    /// return normal of three positions
    static auto
    normal(const vector3f& p0, const vector3f& p1, const vector3f& p2)
    {
    ...
    {
        return (position(vertex(t, 0)) + position(vertex(t, 1))
                + position(vertex(t, 2)))
               * (1.f / 3);
    }
    ...
    triangle_index
    get_adjacent_triangle(triangle_index triangle, unsigned edge) const
    {
        return triangle_adjacency[triangle.get_index()][edge];
    }
    triangle_index get_triangle_of_vertex(vertex_index vtx) const
    {
        return vertex_triangle_adjacency[vtx.get_index()];
    }
    /// Given a triangle and one of its vertices return the corner index of the
    /// vertex.
  ...
  protected:
    ...
    std::vector<uint8_t>
        righthanded; ///< Is local coordinate system righthanded?
    std::vector<std::array<triangle_index, 3>>
        triangle_adjacency; ///< Adjacency information, computed on demand
    std::vector<triangle_index>
        vertex_triangle_adjacency; ///< Adjacency information, computed on
                                   ///< demand
    ...
    std::string name; ///< optionally a mesh can have a name (for use in model
