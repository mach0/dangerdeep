...
{
  public:
    /// type of shader (vertex or fragment, later maybe geometry shader with
    /// GF8800+)
    enum type
    {
        VERTEX,
...
class glsl_program
{
  public:
    ...
    void
    set_gl_texture(const texture& tex, unsigned loc, unsigned texunit) const;
...
class glsl_shader_setup
{
  public:
    ...
    /// set up texture for a particular shader name
    void
    set_gl_texture(const texture& tex, unsigned loc, unsigned texunitnr) const
    {
