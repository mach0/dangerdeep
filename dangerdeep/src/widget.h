...
class widget
{
  protected:
    ...
    image* background; // if this is != 0, the image is rendered in the
                       // background, centered
    const texture*
        background_tex; // drawn as tiles if != 0 and no image defined
    bool enabled;
    std::vector<std::unique_ptr<widget>> children;
    int retval;
    bool closeme; // is set to true by close(), stops run() the next turn
    mutable bool redrawme; // is set to true by redraw(), cleared each time
                           // draw() is called
    ...
    virtual void
    draw_area_col(int x, int y, int w, int h, bool out, color c) const;
  ...
  public:
    ...
    */
    ...
    {
        const widget* source;
        mouse_release_event(const widget* _source) : source(_source) { }
    };
  ...
  public:
    ...
    static void set_theme(std::unique_ptr<theme> t)
    {
        ...
        unsigned direction = 2)
    {
        ...
        add_child_near_last_child_generic(
            std::unique_ptr<widget>(std::move(w)), distance, direction);
        return ref;
    }
    ...
    virtual bool is_mouse_over(const vector2i& m) const
    {
        return is_mouse_over(m.x, m.y);
    }
    virtual void draw() const;
    virtual bool compute_focus(int mx, int my);
    virtual bool compute_focus(const vector2i& m)
    {
        return compute_focus(m.x, m.y);
    }
    virtual bool compute_mouseover(int mx, int my);
    virtual bool compute_mouseover(const vector2i& m)
    {
        return compute_mouseover(m.x, m.y);
    }
    ...
    // Note! such a function is a bad idea, as image is not unref'd then at the
    // cache! virtual void set_background(const image* b) { background = b;
    // background_tex = 0; }
    ...
    virtual void
    on_drag(vector2i position, vector2i motion, mouse_button_state btnstate)
    {
    }
    ...
    virtual void
    close(int val); // close this widget (stops run() on next turn, returns val)
    ...
    static void
    ref_all_backgrounds(); // for all stacked widgets, ref backgrounds
    static void
    unref_all_backgrounds(); // for all stacked widgets, unref backgrounds
};
...
class widget_caller_button : public widget_button
{
  ...
  public:
    ...
    widget_caller_button(std::function<void(Types...)> func_, Types... args_) :
        widget_button(0, 0, 0, 0, std::string{}, nullptr), func(func_),
        args(args_...)
    {
    }
    void on_release() override
...
class widget_list : public widget
{
  protected:
    ...
    int columnwidth; // in pixels, translates tabs to column switches, set -1
...
class widget_fileselector : public widget
{
  ...
  public:
    ...
    {
        return current_path->get_text() + current_filename->get_text();
    }
    void listclick();
};
