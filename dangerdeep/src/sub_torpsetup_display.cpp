...
bool sub_torpsetup_display::handle_mouse_motion_event(const mouse_motion_data& m)
{
	...
	if (m.left()) {
		if (which_element_is_turned != -1) {
			...
			elem.set_value(angle(elem.get_value() + m.relative_motion.x * 50.0).value());//fixme scaling
			switch (which_element_is_turned) {
			case et_primaryrangeknob:
				element_for_id(et_primaryrangedial).set_angle(elem.get_value());
				tbsetup.primaryrange = element_for_id(et_primaryrangedial).get_value(); break;
			...
			case et_rundepthknob:
				...
				tbsetup.rundepth = element_for_id(et_rundepth).get_value(); break;
			default:	break;
			}
		}
	}
...
void sub_torpsetup_display::display() const
{
	...
	// If we had a separate virtual method that gathers data from game and stores it in the displays
	// we wouldn't need to make value mutable.
	...
	element_for_id(et_secondaryrange).set_phase(tbsetup.short_secondary_run ? 0 : 1);
	...
	element_for_id(et_secondaryrangeptr).set_value(helper::mod(gm.get_time(),1600.0));//fixme tbsetup.secondaryrange atm only 800/1600 what was realistic?
	element_for_id(et_primaryrangeptr).set_value(helper::mod(gm.get_time(),1600.0));//fixme tbsetup.primaryrange
	// fixme no element for the LUT angle, the angle that LUT turns to after first run. Or is THAT the turn angle and LUT turns always 180 between runs?
	// FAT can do 90° or 180° turns but we can only use 180°.
	// Primary run length is 1200 or 1900m we have 800m and 1600m.
	// The turn angle is the LUT angle to turn to after initial run.
	// Primary range can be chosen as what? We have 0-1600m.
	draw_elements();
}
