...
void tdc::simulate(double delta_t)
{
    ...
    // note: this value can only be > 1 if target is faster than torpedo and the
    // AoB is bad in that case, just keep old angle, do not "reset" dial to zero
    // (the old analogue technology wouldn't have done that either)
    ...
    if (fabs(sinrelleadangle) <= 1.0)
    {
        ...
        torpedo_runtime  = (angleonthebow.sin() * target_distance)
                          / (torpedo_speed * aob_on_hit.sin());
        ...
        // 95m for radius of turn (a smaller relative lead angle would mean a
        // smaller radius... was this handled in reality??)
        double c = 37 + 9.5 + 95;
