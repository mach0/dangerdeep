...
bool sub_tdc2_display::handle_mouse_button_event(const mouse_click_data& m)
{
    ...
    if (m.down() && m.left())
    {
        ...
        const auto nrtubes =
            sub->get_nr_of_bow_tubes() + sub->get_nr_of_stern_tubes();
...
void sub_tdc2_display::display() const
{
    ...
    unsigned selected_tube =
        dynamic_cast<const submarine_interface&>(ui).get_selected_tube();
    // draw tubes if ready
    ...
    for (unsigned i = 0; i < 6; ++i)
    {
        ...
        {
            if (selected_tube != i
                || gm.get_time() > tubeselected_time + blink_duration
                || (unsigned(floor(
                        (gm.get_time() - tubeselected_time)
                        / blink_period_duration))
                    & 1))
            {
                visible = true;
            }
    ...
    element_for_id(et_firebutton)
        .set_visible(
            player->is_tube_ready(selected_tube) && TDC.solution_valid());
    ...
    element_for_id(et_leadangle_main)
        .set_value(leadangle.value()); // fixme order?!
    element_for_id(et_leadangle_refinement)
        .set_value(
            helper::mod(leadangle.value(), 10.0)
            * 36.0); // fixme 1 degree in 1/10th or 10 degrees?
    ...
    element_for_id(et_targetcourse_refinement)
        .set_value(helper::mod(tgtcourse, 10.0) * 36.0);
    // fixme later integrate them
    // element_for_id(et_targetcourse_main_mkr).set_value(tgtcourse);
    // element_for_id(et_targetcourse_refinement_mkr).set_value(helper::mod(tgtcourse,
    // 10.0) * 36.0);
    // target range
    element_for_id(et_targetdist_ptr).set_value(TDC.get_target_distance());
    // compute non-linear dial value fixme wtf - we must offer that in
    // user_display!!!
    // tgtrange = sqrt(12.61855670103 * tgtrange - 3685.567010309);
    element_for_id(et_brightness)
        .set_value(45.0); // fixme show some sensible value
