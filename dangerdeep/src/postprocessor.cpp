...
postprocessor::postprocessor() :
    filter(
        get_shader_dir() + "postp_null.vshader",
        get_shader_dir() + "postp_filter.fshader"),
    combine(
        get_shader_dir() + "postp_null.vshader",
        get_shader_dir() + "postp_combine.fshader"),
    combine2(
        get_shader_dir() + "postp_null.vshader",
        get_shader_dir() + "postp_combine2.fshader"),
    hipass(
        get_shader_dir() + "postp_null.vshader",
        get_shader_dir() + "postp_highpass.fshader")
...
void postprocessor::bloom()
{
    ...
    glsl_shader_setup::default_tex->set_uniform(
        glsl_shader_setup::loc_t_color, colorf(color::white()));
    glsl_shader_setup::default_tex->set_gl_texture(
        *scene_t, glsl_shader_setup::loc_t_tex, 0);
    ...
    if (use_hqsfx)
    {
        ...
        filter.set_uniform(
            loc,
            vector3f(
                5.0 / 16.0, 6.0 / 16.0, 5.0 / 16.0)); // normalised kernel parms
...
void postprocessor::hdr()
{
    ...
    glsl_shader_setup::default_tex->set_uniform(
        glsl_shader_setup::loc_t_color, colorf(color::white()));
    glsl_shader_setup::default_tex->set_gl_texture(
        *h_textures[0], glsl_shader_setup::loc_t_tex, 0);
