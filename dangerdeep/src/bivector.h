...
template <class T> void bivector<T>::resize(const vector2i& newsz, const T& v)
{
    ...
    /*
        ...
        // fixme: maybe special case when newsz.x*newsz.y <=
...
template <class T> bivector<T> bivector<T>::upsampled(bool wrap) const
{
    ...
    */
    ...
    if (wrap)
    {
        ...
        for (int y = 0; y < datasize.y; ++y)
        {
            ...
            result.at(2 * datasize.x - 1, 2 * y) =
                T((at(datasize.x - 1, y) + at(0, y)) * 0.5);
        }
    }
    else
    {
    ...
    if (wrap)
    {
        ...
        for (int x = 0; x < resultsize.x; ++x)
        {
            result.at(x, resultsize.y - 1) =
                T((result.at(x, resultsize.y - 2) + result.at(x, 0)) * 0.5);
        }
    }
    return result;
}
    ...
    */
    ...
    if (!force_even_size)
    {
        ...
        if ((datasize.x & datasize.y) & 1)
        {
            ...
            result.at(newsize.x, newsize.y) =
                at(datasize.x - 1, datasize.y - 1);
        }
    }
    return result;
}
...
bivector<T>& bivector<T>::add_tiled(const bivector<T>& other, const T& scal)
{
    bivector_FOREACH_XY(
        at(x, y) +=
        other.at(x & (other.datasize.x - 1), y & (other.datasize.y - 1))
        * scal;) return *this;
}
