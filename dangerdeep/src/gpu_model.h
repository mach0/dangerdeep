...
protected:
	...
	struct material
	{
		...
		std::vector<std::pair<const texture*, sampler::type>> textures_and_samplers;	///< For every location the texture/sampler for current layout
	};
	std::vector<material> materials;		///< The materials used
	object_store<texture> texture_store;		///< All textures of the model in a store
	std::string current_layout;			///< The current layout that textures are set up for

	// fixme set caustic animation time once globally by game time! have caustic animation number here?
	// or fraction of animation [0...1] as float here? with static function to set it?
	// or have that in scene class???
	...
	static std::unordered_map<basic_shader_feature, program> default_programs;	///< Default shader programs
	static texture_array caustics;	///< texture array with caustic data for default programs
