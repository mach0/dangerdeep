...
template <typename D, typename S> inline void convert_pm1(D& d, S s)
{
    d = D(s);
}
template <typename D> inline void convert_pm1(D& d, uint8_t s)
{
    d = D(D(s) - D(128)) / D(127);
}
template <typename S> inline void convert_pm1(uint8_t& d, S s)
{
    d = uint8_t(s * S(127) + S(128));
}
template <> inline void convert_pm1<uint8_t, uint8_t>(uint8_t& d, uint8_t s)
{
    d = s;
}
template <typename D, typename S> inline void convert_01(D& d, S s)
{
    d = D(s);
}
template <typename D> inline void convert_01(D& d, uint8_t s)
{
    d = D(s) / D(255);
}
template <typename S> inline void convert_01(uint8_t& d, S s)
{
    d = uint8_t(s * S(255));
}
template <> inline void convert_01<uint8_t, uint8_t>(uint8_t& d, uint8_t s)
{
    d = s;
}
...
T interpolate(const T& a, const T& b, const S& v)
{
    return T(a * (S(1) - v) + b * v);
}

/// check if value is power of two
inline bool is_power2(unsigned x) { return (x & (x - 1)) == 0; }

/// compare with tolerance value
template <typename T>
bool is_equal_with_tolerance(const T a, const T b, const T tolerance)
{
...
template <typename T> T frac(const T a) { return a - std::floor(a); }
...
/// round value
template <typename T> T round(const T v) { return std::floor(v + T(0.5)); }
...
template <class C, typename T> bool contains(const C& cnt, const T& value)
{
    return std::find(begin(cnt), end(cnt), value) != end(cnt);
}

/// remove all values for that predicate returns true and compacts the
/// container.
template <class C, class P> void erase_remove_if(C& cnt, P predicate)
{
    cnt.erase(
        std::remove_if(std::begin(cnt), std::end(cnt), predicate),
        std::end(cnt));
}
...
/// any_of with range
template <class C, class P> bool any_of(C& cnt, P predicate)
{
    return std::any_of(begin(cnt), end(cnt), predicate);
}

/// any_of with range
template <class C, class P> bool all_of(C& cnt, P predicate)
{
    return std::all_of(begin(cnt), end(cnt), predicate);
}

/// any_of with range
template <class C, class P> bool none_of(C& cnt, P predicate)
{
...
template <typename T>
void count(const T start, const T limit, std::function<void(const T)> func)
{
...
/// count and call for every number
template <typename T>
void count(const T number, std::function<void(const T)> func)
{
    count(T{}, number, func);
}

/// find object from range with least value (distance)
template <template <typename, typename...> class C, typename D, typename T>
T nearest(const C<T>& cnt, std::function<D(const T&)> dist)
{
...
inline std::string replace_first(
    std::string s, const std::string& pattern, const std::string& replace_with)
{
