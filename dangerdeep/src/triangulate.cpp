 ...
 * Start from vertex with largest angle and find nearest vertices that is within
 * beam limited by part of angle and maximum total angle. That way we have one
 * edge connecting two vertices and we partition the polygon in two parts.
 * Repeat for both parts until only triangles result.
 * Gives a stable, fast and good looking result.
 * We can also fill the space inside the polygon by creating a regular grid
 * raster, that can be done by rasterizing the polygon edges and filling in
 * remaining spaces.
 */
...
while point list contains more than three points
    ...
    if p_i, p_i+1, p_i+2 don't form a triangle (angle at p_i+1 is >= 180�),
i:=i+1, continue for all j in from [i+3, i[ (cyclic indices!) check if p_j is in
triangle p_i, p_i+1, p_i+2 if all points are outside, then add triangle
...
vector<unsigned> triangulate::compute(const vector<vector2>& vertices)
{
    ...
    // fixme 2: use delaunay condition to check triangles. But maybe the
    ...
    int haengt =
        0; // fixme: hack to avoid lock ups. why do they occour? reasons maybe:
           ...
           // are degenerated or too short. OCCOUR, fixme, SEE COASTSEGMENT.CPP
           // 2) the polygon is self-intersecting. ???? CHECK
           // 3) polygon is inverted (hole), that means polylines are all in
           // wrong direction AVOIDED, FIXED 4) if several points are on one
           // line a,b,c,d and one is beside (e) and a triangle d,e,a is formed,
