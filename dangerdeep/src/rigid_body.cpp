...
fixme compute_force_and_torque: replace by function that computes positions and forces applied there?
torque can be computed internally then.
that way we don't need to take the orientation out of here...
only offer function that gets a list of forces applied in world space

fixme: medium drag causing sideways movement has to be implemented correctly in sea_object!
...
Force comes from screws/rudders etc. and also from the medium pushing on the body.
This explains why moving objects can do curves even if no more force applied:
The surrounding medium causes drag that slows the body down but because of the shape
of the object it can also generate linear momentum that is not in move direction causing
the body to change direction of movement. For example ships or airplanes move forward,
and when turning change their orientation. They sill move forward but the medium will
bounce off their side causing a push to the side, so the drag will not only slow them
down but also make them move sideways. This leads to a linear movement that is following
the changed orientation. This explains how bodies can change their direction only with
internal force/energy.
For ships we simulate voxels that generate lift values causing the ship to change orientation
with water height values.
...
void rigid_body::set_mass_and_inertia_tensor(mass1d mass_, const matrix3& inertia_tensor_)
{
	mass = mass_;
...
void rigid_body::compute_helper_values()
{
	...
	velocity3d w = (inertia_tensor_inv * angular_momentum.rotate(orientation.conj())) / mass;
...
void rigid_body::simulate(duration delta_time, const force3d& force, const torque3d& torque)
{
	...
	const velocity3d w = (inertia_tensor_inv * angular_momentum.rotate(orientation.conj())).rotate(orientation) / mass;
...
void rigid_body::simulate(duration delta_time, std::initializer_list<std::pair<vector3, vector3>> local_forces)
{
	vector3 local_force;
	...
	simulate(delta_time, orientation.rotate(local_force), orientation.rotate(local_torque));
}



/// compute linear velocity of the rigid body at a given global position
velocity3d rigid_body::compute_linear_velocity(const vector3& position_global) const
{
...
double rigid_body::compute_collision_response_value(const vector3& collision_pos, const vector3& N) const
{
	//WTF is N and what is this formula about? some force acting locally gives force/torque.
	//why not apply collision force?!
	const vector3 relative_position = collision_pos - position;
	return (N * orientation.rotate(inertia_tensor_inv * orientation.conj().rotate(relative_position.cross(N))).cross(relative_position)) / mass.value;
}



/// apply impulse caused by a collision to the rigid body
void rigid_body::apply_collision_impulse(const vector3& collision_pos, const momentum3d& J)
{
	const vector3 relative_position = collision_pos - position;
	linear_momentum += J;
