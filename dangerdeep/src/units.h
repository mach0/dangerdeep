...
enum class physical_unit
{
	...
	momentum,	// force*time or torque*time = mass*velocity, kg*m/s = N*s, also called impulse
...
template<typename T, physical_unit P> struct physical_value
{
	...
	/// Rotate 3d values
	auto rotate(const quaternion& q) const { return physical_value<T, P>(q.rotate(value)); }
	/// Cross product for 3d values
	auto cross(const vector3& v) const { return physical_value<T, P>(value.cross(v)); }
	/// Add values
	auto operator+= (const physical_value<T, P>& other) { value += other.value; }
	/// Add values
	auto operator+ (const physical_value<T, P>& other) const { return physical_value<T, P>(value + other.value); }
	/// Compare
	auto operator< (const physical_value<T, P>& other) const { return value < other.value; }
};
/// transform 3d values (also rotation)
template<physical_unit P> auto operator* (const matrix3& m, const physical_value<vector3, P>& v) { return physical_value<vector3, P>(m * v.value); }
...
auto duration_between(time_point t0, time_point t1) { return duration(t1.value - t0.value); }
...
template<typename T> auto operator* (const physical_value<T, physical_unit::velocity>& v, duration d) {
	return v.value * d.value;
}

/// Angular velocity
typedef physical_value<double, physical_unit::angular_velocity> angular_velocity;
angle operator* (angular_velocity av, duration d) { return angle(av.value * d.value); }

/// Force in newtons
typedef physical_value<double, physical_unit::force> force1d;
typedef physical_value<vector3, physical_unit::force> force3d;
template<typename T> auto operator* (const physical_value<T, physical_unit::force>& v, duration d) {
...
template<typename T> auto operator* (const physical_value<T, physical_unit::acceleration>& v, duration d) {
...
template<typename T> auto operator* (const physical_value<T, physical_unit::torque>& t, duration d) {
...
template<typename T> auto operator* (const physical_value<T, physical_unit::acceleration>& a, mass1d m) {
	return physical_value<T, physical_unit::force>(a.value * m.value);
}
/// Get linear momentum
template<typename T> auto operator* (const physical_value<T, physical_unit::velocity>& v, mass1d m) {
	return physical_value<T, physical_unit::momentum>(v.value * m.value);
}
/// Get velocity from momentum
template<typename T> auto operator/ (const physical_value<T, physical_unit::momentum>& v, mass1d m) {
	return physical_value<T, physical_unit::velocity>(v.value * (1.0/m.value));
}
...
force3d gravity_force(mass1d m) { return force3d(vector3(axis::neg_z) * m.value * constant::GRAVITY); }

/// Distance in meters
typedef physical_value<double, physical_unit::distance> distance;
distance get_distance(const vector3& a, const vector3& b) { return distance((a - b).length()); }
velocity1d operator/ (distance d, duration t) { return velocity1d(d.value / t.value); }

/// 2d area
typedef physical_value<double, physical_unit::area> area2d;

/// Later have some class that checks value is between 0 and 1. Maybe add class for that, no physical unit.
