...
class videoplay : public thread
{
  public:
    struct framebuffer
    {
        ...
        framebuffer() :
            width(0), height(0), empty(true), aspect_ratio(0.0f),
            time_stamp(-1.0)
        {
        }
    };
...
videoplay::videoplay(const std::string& filename, unsigned queue_len) :
    thread("videoplay"), fb_queue(queue_len), playback_starttime(-1.0),
    playback_started(false), current_playtime(0.0), real_time(0.0),
    vstr_idx(-1), ictx(0), ifmt(0), codec(0), picture(0), frame_nr(0),
    eof(false), next_frame_pts(-1.0),
    myshader(
        get_shader_dir() + "videoplay.vshader",
        get_shader_dir() + "videoplay.fshader")
{
    ...
    try
    {
        ...
        if (res < 0)
            THROW(
                error, std::string("error finding stream info in ") + filename);
        ...
        if (avcodec_open(context, codec) < 0)
        {
            ...
            if (context->thread_opaque)
                avcodec_thread_free(context);
...
{
    avcodec_close(
        ictx->streams[vstr_idx]->codec); // also calls avcodec_thread_free()
    av_freep(&picture);
    av_close_input_file(ictx);
    av_free_packet(&ipkt);
}
...
void videoplay::loop()
{
    ...
    double stamp = double((ipkt.pts == AV_NOPTS_VALUE) ? ipkt.dts : ipkt.pts)
                   * ictx->streams[vstr_idx]->time_base.num
                   / ictx->streams[vstr_idx]->time_base.den;
    ...
    ++frame_nr;
    // do this only when pts reordering is needed (always except for AVIs and
    // MP4s) or always use DTS, that is always linear can we request that modus
    // from context->b_frame
    ...
    fb.height       = h;
    fb.empty        = false;
    fb.aspect_ratio = double(ctx->sample_aspect_ratio.num * w)
                      / (ctx->sample_aspect_ratio.den * h);
    fb.time_stamp = stamp;
...
void videoplay::display_loop(double delta_time)
{
    ...
    unsigned nr_full_buffers = 0;
    {
        ...
        if (!decoded_pictures.empty())
        {
            ...
            if (playback_started
                && fb_queue[bufnr].time_stamp
                       <= (real_time + playback_starttime))
...
{
    if (tex_y.get()
        && (tex_y->get_width() != fb.width || tex_y->get_height() != fb.height))
    {
        tex_y.reset();
        tex_uv.reset();
    }
    if (!tex_y.get())
    {
        tex_y.reset(new texture(
            fb.width,
            fb.height,
            GL_LUMINANCE,
            texture::LINEAR,
            texture::CLAMP));
        // no UV filtering, so use nearest! - we could place UV in planes, not
        // interleaved...
        tex_uv.reset(new texture(
            fb.width / 2,
            fb.height / 2,
            GL_LUMINANCE_ALPHA,
            texture::NEAREST,
    ...
    tex_uv->sub_image(
        0, 0, fb.width / 2, fb.height / 2, fb.uv, GL_LUMINANCE_ALPHA);
    myshader.use();
...
int mymain(std::vector<string>& args)
{
    ...
    system_interface::parameters params(
        1.0, 30000.0 + 500.0, res_x, res_y, fullscreen);
