    ...
    unsigned nr_segs)
{
    ...
    primitives::cylinder_z(
        radius_bottom, radius_top, -1.5, height, alpha, tex, u_scal, nr_segs)
        .render();
}
...
auto water_splash::compute_height(double t) const -> double
{
    ...
    {
        double q = t - risetime;
        return riseheight - constant::GRAVITY * 0.5 * q * q;
    }
}
...
void water_splash::simulate(double delta_time, game& gm)
{
    if (!is_reference_ok())
    {
        return;
...
void water_splash::display() const
{
    ...
    if (lifetime - resttime > 0.5)
    {
        ...
        render_cylinder(
            rb, rt, compute_height(lifetime - resttime - 0.5) * 1.2, a, tex);
    }
    if (resttime > 0)
    {
