...
water::water(double tm) :
    mytime(tm), wave_phases(cfg::instance().geti("wave_phases")),
    ...
    wave_resolution_shift(ulog2(wave_resolution)), wavetile_data(wave_phases),

    owg(wave_resolution,
        vector2f(1, 1),          // wind direction
        12 /*12*/ /*10*/ /*31*/, // wind speed m/s. fixme make dynamic
                                 // (weather!)
        wave_resolution
            * (1e-8) /* roughly 2e-6 for 128 */, // scale factor for heights.
                                                 // depends on wave resolution,
                                                 // maybe also on tidecycle time
        wavetile_length,
        wave_tidecycle_time),

    geoclipmap_resolution(
        cmpdtl(cfg::instance().geti("water_detail"))), // should be power of two
    ...
    patches(1 + (geoclipmap_levels - 1) * 8 * 3 * 3 + 4 /*horizon*/)
{
    ...
    for (unsigned j = 1; j < geoclipmap_levels; ++j)
    {
        ...
        for (int y = -1; y <= 1; ++y)
        {
            for (int x = -1; x <= 1; ++x)
            {
                ...
                patches[pn + 0] = std::make_unique<geoclipmap_patch>(
                    geoclipmap_resolution, j, 0x80, 0, 0, N4 + x, N4 + y);
                patches[pn + 1] = std::make_unique<geoclipmap_patch>(
                    geoclipmap_resolution, j, 0x01, N4 + x, 0, N2, N4 + y - 1);
                patches[pn + 2] = std::make_unique<geoclipmap_patch>(
                    geoclipmap_resolution, j, 0x40, N34 + x, 0, N4 - x, N4 + y);
    ...
    patches[pn + 0] = std::make_unique<geoclipmap_patch>(
        geoclipmap_resolution, geoclipmap_levels - 1, 0x01);
    patches[pn + 1] = std::make_unique<geoclipmap_patch>(
        geoclipmap_resolution, geoclipmap_levels - 1, 0x02);
    patches[pn + 2] = std::make_unique<geoclipmap_patch>(
        geoclipmap_resolution, geoclipmap_levels - 1, 0x04);
    patches[pn + 3] = std::make_unique<geoclipmap_patch>(
        geoclipmap_resolution, geoclipmap_levels - 1, 0x08);
    ...
    // 2004/04/25 Note! decreasing the size of the reflection map improves
    // performance on a gf4mx! (23fps to 28fps with a 128x128 map to a 512x512
    // map) Maybe this is because of some bandwidth limit or cache efficiency of
    // the gf4mx. Reflections don't look very good on ocean waves because
    // they're too high for the distorsion to work correct. They need real
    // environment mapping, but this lacks local reflections. Best example: big
    // waves in front of the coast should hide the coast (view is blocked) but
    // instead mirror it. Maybe this is because we don't clip the mirror image
    // at z=0 ? No, test showed that it isn't looking right anyway! In fact,
    // terrain is seen only at rare moments, vessels' reflections can't be seen
    // very well in rough seas and sun/moon/stars don't give large reflections.
    // They're used as fake specular mapping for now. Only explosions/fires at
    // night would be seen as reflections. So why use them at all? they're
    // rather costly! make them configureable? fixme fixme: make size
    // configurable in parts of screen resolution
    ...
    // fixme: make ^ that configureable! reflection doesn't need to have that
    // high detail... fixme: auto mipmap?
    reflectiontex = std::make_unique<texture>(
        rx, ry, GL_RGB, texture::LINEAR, texture::CLAMP, true);

    log_info(
        "wave resolution " << wave_resolution
                           << " (shift=" << wave_resolution_shift << ")");
    log_info("reflection image size " << rx << "x" << ry);
    log_info("water detail: " << geoclipmap_resolution);

    // initialize shaders
    glsl_water = std::make_unique<glsl_shader_setup>(
        get_shader_dir() + "water.vshader", get_shader_dir() + "water.fshader");
    glsl_under_water = std::make_unique<glsl_shader_setup>(
        get_shader_dir() + "under_water.vshader",
    ...
    loc_w_reflection_transform =
        glsl_water->get_uniform_location("reflection_transform");
    glsl_under_water->use();
    loc_uw_noise_xform_0 =
        glsl_under_water->get_uniform_location("noise_xform_0");
    loc_uw_noise_xform_1 =
        glsl_under_water->get_uniform_location("noise_xform_1");
    ...
    loc_uw_upwelltopbot =
        glsl_under_water->get_uniform_location("upwelltopbot");
    ...
    if (framebufferobject::supported())
    {
        ...
        reflectiontex_fbo =
            std::make_unique<framebufferobject>(*reflectiontex, true);
        foamamounttex_fbo =
            std::make_unique<framebufferobject>(*foamamounttex, false);
    }
    ...
    for (unsigned y = 0; y < perimetertexs; ++y)
    {
        ...
        for (unsigned x = 0; x < perimetertexs; ++x)
        {
            ...
            if (d < perimetertexs / 2)
            {
                ...
                {
                    a = uint8_t(
                        255 * (perimetertexs / 2 - d) / perimetertexborder);
                }
                else
                {
                    a = 255;
    ...
    /*
      Idea:
      Computing one Height map with FFT takes roughly 2 ms on a 1800Mhz PC
      (64x64). It has to be done 25 times per second, taking just 50ms or 5% of
      all time. So the FFT heights could get computed on the fly, leading to
      more realistic results, since they don't need to be cyclic. And we can
      change the fft parameters at run-time (like switching weather). Also much
      memory is saved. With 256 Phases of 64x64 each we have 1M with 1 byte per
      height or 4M as we have now. With 128x128 fft resolution (much better than
      64x64) that would be already 4M/16M. Heigher resolution fft could also be
      used as sub-noise for shader display or as additional sub-detail
      (self-similar noise). With on-the-fly fft we could give a cyclic value of
      1-2 minutes. Just blend the fft coefficients between two levels for
      weather changes, like with the clouds.
    */

    // multithreaded construction of water data (faster).
    // spawn 1 more thread (or 3 on 4-core cpus, but two threads are already
    // fast enough)
    ...
    unsigned phase_add)
{
    ...
    {
        generate_wavetile(
            myowg, wave_tidecycle_time * i / wave_phases, wavetile_data[i]);
    }
}
    ...
    bool under_water) const
{
    ...
    /* how to compute that.
       ...
       To achieve this we need to know how many tiles we move horizontally and
       vertically for the path. This are numbers A and B. We move with velocity
       V (meters/second), thus we get as time t for the path: t = tile_size *
       sqrt(a*a + b*b) / V and for the direction vector of movement: D =
       (V/sqrt(a*a + b*b)) * (a, b) Example, if we would like to move 2 tiles
       right and 5 down, with 0.5m/s, we get t = 256m * sqrt(2*2+5*5) / 0.5m/s =
       2757.2s and D = (0.186, 0.464) So we have to take mytime module t and use
       that as multiplier with D to get the current position (plus initial
       offset S). In the shader the vertex position is multiplied with factor z
       here and then xy are added. So z translates meters to texture
       coordinates. For noise #0 scale is 8/256m, so we have 8 tiles of the
       texture per wave tile. So one noise tile is 256m/8 = 1/z = 32m long. That
       is the tile size we have to use for the computation above. 32m with
       V=2m/sec -> t = 86.162 and D = (0.743, 1.857)
    */
    static const int a_0 = 2, b_0 = 5;
    static const int a_1 = -4, b_1 = 3;
#ifdef WIN32
    // windows lacks sqrt(int) ?!?! TODO: casting to float ok as a general rule?
    // -- matt
    static const double s_0 = sqrt((float) (a_0 * a_0 + b_0 * b_0)),
                        s_1 = sqrt((float) (a_1 * a_1 + b_1 * b_1));
#else
    static const double s_0 = sqrt(a_0 * a_0 + b_0 * b_0),
                        s_1 = sqrt(a_1 * a_1 + b_1 * b_1);
#endif
    static const double V_0 = 2.0, V_1 = 1.0;
    // maybe: remove hardwired scale factors of 8 and 32, but looks best with
    // that values.
    static const double t_0 = wavetile_length / 8.0 * s_0 / V_0,
                        t_1 = wavetile_length / 32.0 * s_1 / V_1;
    // need to divide by noise tile size here too (tex coordinates are in
    // [0...1], not meters)
    static const vector2f D_0 =
        vector2f(a_0, b_0) * (V_0 / s_0 / 32.0); // noise tile is 256/8=32m long
    static const vector2f D_1 =
        vector2f(a_1, b_1) * (V_1 / s_1 / 8.0); // noise tile is 256/32=8m long
    ...
    if (under_water)
    {
        glsl_under_water->set_uniform(
            loc_uw_noise_xform_0, noise_0_pos.xyz(wavetile_length_rcp * 8.0f));
        glsl_under_water->set_uniform(
            loc_uw_noise_xform_1, noise_1_pos.xyz(wavetile_length_rcp * 32.0f));
        glsl_under_water->set_gl_texture(*water_bumpmap, loc_uw_tex_normal, 0);
    }
    else
    {
        glsl_water->set_uniform(
            loc_w_noise_xform_0, noise_0_pos.xyz(wavetile_length_rcp * 8.0f));
        glsl_water->set_uniform(
            loc_w_noise_xform_1, noise_1_pos.xyz(wavetile_length_rcp * 32.0f));
        glsl_water->set_gl_texture(*water_bumpmap, loc_w_tex_normal, 0);
    }

    // set up texture matrix, so that texture coordinates can be computed from
    // position.
    if (!under_water)
    {
        ...
        glsl_water->set_uniform(
            loc_w_foam_transform,
            matrix4::diagonal(1.0f / noisetilescale, 1.0f / noisetilescale, 1)
                * matrix4::trans(transl.x, transl.y, 0));
    }

    /* 2005/06/28:
       fixme: with shaders there are some errors, reflections look strange (dark
       blue reflection in the near etc. foam experiments showed that the
       texcoords used to fetch the reflection texels are wrong when looking to a
       certain direction. foam texture coordinates are also wrong then (use a
       checker pattern for foam amount to easily see it). maybe the matrices are
       wrong? yes reflection_projmvmat is not right, some matrix changes happen
       AFTER it is computed so that it doesn't match the current proj/modl
       matrices. display() gets the refl_pm parameter, but would not need it.
       fix that matrix! it appears only in freeview mode, not in
       bridge/uzo/glasses mode!
    */

    /* 2005/06/28
       reflection map can be much smaller than screen res, because ocean water
       reflects the scene mostly very fuzzy, details can not be seen... fixme
    */
    ...
    const game& gm, const ship* shp, const vector3& viewpos) const
{
    ...
    */
    ...
    // iterate over stored positions, compute normal for trail for each position
    // and width
    ...
    while (pit != prevposn.end())
    {
        ...
        // amount of foam (density) depends on time (age). foam vanishs after
        // 30seconds
        double age        = tm - pit->time;
        double foamamount = fmax(0.0, 1.0 - age * (1.0 / 30));
        // width of foam trail depends on speed and time.
        // "young" foam is growing to max. width, max. width is determined by
        // speed width is speed in m/s * 2, gives ca. 34m wide foam on each side
        // with 34kts.
        ...
        // y-coord depends on total length somehow, rather on distance between
        // two points. but it should be fix for any position on the foam, or the
        // edge of the foam will "jump", an ugly effect - we use a workaround
        // here to use time as fake distance and take mod 3600 to keep fix y
        // coords.
    ...
    const vector<const ship*>& allships) const
{
    ...
    unsigned afs =
        foamamounttex->get_gl_width(); // size of amount of foam size, depends
                                       // on water grid detail, fixme
    ...
    // clear it , fixme: must be done earlier, there is already drawn something
    // inside the viewport
    glClearColor(0, 0, 0, 0);
    // fixme: clears whole viewport, which kills display with torpedo camera
    // view when not using FBOs...
    glClear(GL_COLOR_BUFFER_BIT);
    glDisable(GL_DEPTH_TEST);

    // draw trails / wave tops

    // wave tops: draw one quad matching the water surface, texture mapped with
    // a "amount-of-foam-of-wave-tile" texture
    // generated from wave heights (could be constant in time, just moving
    // around). indicates foam at high waves.
    ...
    // fixme: needs infos about ships/subs/torps/shells? only accessible in
    // user_interface/freeview_display ship holds previous positions (list of
    // vector2 that are needed for display)
    // as first only subs, draw quad strip along the trail
    // first strip width depends on bow width, then model width, constant with
    // with decaying strength maybe with a texture of grey/black lines or mix to
    // simulate screw foam/hull foam/randomness etc.
    /*
        glRasterPos2i(50, 50);
    ...
    const vector3& viewpos, double max_view_dist, bool under_water) const
{
    ...
    vector2f transl(
        helper::mod(viewpos.x, double(wavetile_length)),
        helper::mod(viewpos.y, double(wavetile_length)));

    // give -viewpos.z to vertex shader for generation of foam projection
    // coordinates the plane z = -viewpos.z is the water plane.
    if (under_water)
    {
        glsl_under_water->use();
    ...
    // fractal noise. Note that level 0 is NOT rendered with 128x128 quads, but
    // only a inner subsection with n*n quads (n is a power of two, e.g. 32). A
    // reasonable detail for the finest level would be 0.25m, giving an 8m tile
    // length for the finest level data.
    ...
    // Which level number is the farest depends on viewing range. Normally we
    // render all levels up to the coarsest, but with fog or other limited
    // visibility we can spare coarser levels here.
    ...
    // to give same precision of values, no matter how big the value of
    // viewpos.x/y is, this can be very high, as its globally in meters.
    vector2 viewpos_mod(
        helper::mod(viewpos.x, double(wavetile_length)),
        helper::mod(viewpos.y, double(wavetile_length)));

    // Note! on fast movement one can see some popping of detail, when boundary
    // between levels moves around the screen. This can be avoided by smoothly
    // blending over height between data of two levels for vertices near the
    // border. This increases either CPU time (when done at the vertex
    // computation) or GPU time (when done in the vertex shader). It makes much
    // more sense to compute that on the CPU, because we would need to transfer
    // (x,y,z) data for each vertex not only for the current but also the _next_
    // level to the GPU. The effect can hardly be seen at normal gameplay, so we
    // ignore it for now.
    ...
    {
        unsigned nr_verts_total =
            geoclipmap_levels * (N + 1) * (N + 1) + 8 /* horizon */;
        ...
        // - amount of foam (1f)
        // --- sum: 6f
        // No! we still need the normal in the vertex shader, to compute the
        // slope dependent color and, more important, the reflection texture
        // coordinate distorsion... so we have 9 floats here, but still not much
        // data... Some papers state that 32bytes per vertex are ideal (ATI
        // cards), so 8f ideal here... normal can be fetched as 2f instead of 3f
        ...
        // maybe: test if writing to mapped memory is faster than storing and
        // copying data (DMA). but this is way fast enough already!
        for (unsigned level = 0 /*minlevel*/; level < geoclipmap_levels;
             ++level)
        {
            ...
            vector2f offset(
                xoff * L_l - viewpos_mod.x, yoff * L_l - viewpos_mod.y);
            ...
            for (unsigned y = 0; y < N + 1; ++y)
            {
                ...
                for (unsigned x = 0; x < N + 1; ++x)
                {
                    ...
                    // would increase number of rendered levels by 3 or 4, but
                    // still ok for memory consumption...
                    ...
                    vertex_data[vertex_data_ptr + 6] =
                        mml.amount_of_foam[lineidx + colidx];
        ...
        // force vertex height on border to horizon faces to zero, normal to
        // (0,0,1).
        vertex_data_ptr =
            (geoclipmap_levels - 1) * (N + 1) * (N + 1) * nr_vert_attr;
        for (unsigned i = 0; i < N + 1; ++i)
        {
            unsigned k[4] = {
                vertex_data_ptr + nr_vert_attr * i,
    ...
    for (unsigned level = 1; level < geoclipmap_levels; ++level)
    {
        ...
        for (unsigned i = 0; i < 8; ++i)
        {
            ...
            const vector2 offset =
                vector2(x_base[j], y_base[k]) * L_l - viewpos_mod;
            ...
            {
                // printf("culled away patch, level=%i
...
float water::get_height(const vector2& pos) const
{
    ...
    float fracy = y - iy;
    ix &=
        (wave_resolution - 1); // avoid rare cases of wrap-around, when x is ==
                               // wave_resolution because of float round errors
...
vector3f water::get_normal(const vector2& pos, double rollfac) const
{
    ...
    float fracx = x - ix;
    float fracy = y - iy;
    // compute the four normals at the corner points and interpolate between
    // them according to fracx/y
    ...
    ocean_wave_generator<float>& myowg, double tiletime, wavetile_phase& wtp)
{
    ...
    // choppy waves don't look right. bug? fixme, with negative values it seems
    // right. check this! -2.0f also looks nice, -5.0f is too much. -1.0f should
    // be ok
	...
	*/
    ...
    double L = wavetile_length / wave_resolution;
    wtp.mipmaps.emplace_back(displacements, heights, wave_resolution_shift, L);
    for (unsigned i = 1; i < mipmap_levels; ++i)
    {
        wtp.mipmaps.emplace_back(
            wtp.mipmaps.back().wavedata,
    ...
    /*
        float maxdispl = 0.0;
        for (vector<vector2f>::const_iterator it = displacements.begin(); it !=
       displacements.end(); ++it) { maxdispl = max(maxdispl, fabs(it->x));
...
void water::compute_amount_of_foam()
{
    ...
    {
        const vector<vector3f>& wd =
            wavetile_data[k % wave_phases].mipmaps[0].wavedata;
        ...
        for (unsigned y = 0; y < wave_resolution; ++y)
        {
            ...
            {
                unsigned xm1 =
                    (x + wave_resolution - 1) & (wave_resolution - 1);
                unsigned xp1    = (x + 1) & (wave_resolution - 1);
                double dispx_dx = (wd[y * wave_resolution + xp1].x
                                   - wd[y * wave_resolution + xm1].x)
                                  * deriv_fac;
                double dispx_dy = (wd[yp1 * wave_resolution + x].x
                                   - wd[ym1 * wave_resolution + x].x)
                                  * deriv_fac;
                double dispy_dx = (wd[y * wave_resolution + xp1].y
                                   - wd[y * wave_resolution + xm1].y)
                                  * deriv_fac;
                double dispy_dy = (wd[yp1 * wave_resolution + x].y
                                   - wd[ym1 * wave_resolution + x].y)
                                  * deriv_fac;
                double Jxx = 1.0 + lambda * dispx_dx;
                double Jyy = 1.0 + lambda * dispy_dy;
                double Jxy = lambda * dispy_dx;
                ...
                // printf("x,y=%u,%u, Jxx,yy=%f,%f Jxy,yx=%f,%f J=%f\n",
                //       x,y, Jxx,Jyy, Jxy,Jyx, J);
                // double foam_add = (J < 0.3) ? ((J < -1.0) ? 1.0 : (J -
                // 0.3)/-1.3) : 0.0;
                ...
                aof[ym1 * wave_resolution + x] +=
                    foam_add * foam_spawn_fac * 0.5;
                aof[yp1 * wave_resolution + x] +=
                    foam_add * foam_spawn_fac * 0.5;
                aof[y * wave_resolution + xm1] +=
                    foam_add * foam_spawn_fac * 0.5;
                aof[y * wave_resolution + xp1] +=
                    foam_add * foam_spawn_fac * 0.5;
            }
        }
        ...
        for (unsigned y = 0; y < wave_resolution; ++y)
        {
            ...
            {
                aof[ptr] = std::max(
                    std::min(aof[ptr], 1.0f)
                        - (decay + decay_rnd * rndtab[(3 * x + 5 * y) % 37]),
                    0.0);
                ++ptr;
            }
        }
        ...
        if (k >= wave_phases)
        {
            wavetile_phase::mipmap_level& mm0 =
                 ...
                 j < wavetile_data[k - wave_phases].mipmaps.size(); ++j)
            {
                ...
                const wavetile_phase::mipmap_level& mm1 =
                    wavetile_data[k - wave_phases].mipmaps[j - 1];
                wavetile_phase::mipmap_level& mm2 =
                    wavetile_data[k - wave_phases].mipmaps[j];
                ...
                for (unsigned y = 0; y < res; ++y)
                {
                    ...
                    {
                        float sum = mm1.amount_of_foam[ptr]
                                    + mm1.amount_of_foam[ptr + 1]
                                    + mm1.amount_of_foam[ptr + 2 * res]
                                    + mm1.amount_of_foam[ptr + 1 + 2 * res];
...
{
    // update texture with glTexSubImage2D, that is faster than to re-create the
    // texture
...
{
    // do all the tasks here that should happen regularly, like recomputing new
    // water or noisemaps
    mytime = tm;

    unsigned pn =
        unsigned(wave_phases * myfrac(tm / wave_tidecycle_time)) % wave_phases;
...
void water::set_refraction_color(const colorf& light_color)
{
    ...
    // so this will be computed implicitly. Environment can also change water
    ...
    // dim green.
    colorf wavetop =
        light_color.lerp(colorf(color(0, 0, 0)), colorf(color(49, 83, 94)));
    colorf wavebottom =
        light_color.lerp(colorf(color(0, 0, 0)), colorf(color(29, 56, 91)));
...
void water::refltex_render_bind() const
{
    ...
    {
        glViewport(
            0, 0, reflectiontex->get_width(), reflectiontex->get_height());
    }

    // clear depth buffer (not needed for sky drawing, but this color can be
    // seen as mirror image when looking from below the water surface (scope)
    // fixme: clear color with upwelling color!)
    ...
    resolution_shift(res_shift), sampledist(sampledist_)
{
    ...
    for (unsigned y = 0; y < resolution; ++y)
    {
        ...
        {
            vector3f sum = wd[ptr] + wd[ptr + 1] + wd[ptr + 2 * resolution]
                           + wd[ptr + 1 + 2 * resolution];
...
void water::wavetile_phase::mipmap_level::compute_normals()
{
    ...
    // if we render TRIANGLE_STRIPs, we have a determined tesselation and thus
    // can compute correct normals. compute normals for each face (triangles!)
    // and then per vertex. Triangle strip scheme: v0 --- v2 --- v4 |    / | / |
    // |   /  |   /  |
    // |  /   |  /   |
    // v1 --- v3 --- v5
    ...
    // fixme: for higher levels the computed data is not used, as mipmaps are
    // generated by glu. however this data should be much better! but we can't
    // feed it to texture class yet
    ...
    use_fan(false)
{
    ...
    if (border & 0x0f)
    {
        ...
        unsigned parts = (border & 0x05)
                             ? columns
                             : rows; // either 0x4 or 0x1, bottom or top
    ...
    for (unsigned y = 0; y < rows; ++y)
    {
        ...
        else
        {
            ...
            for (unsigned x = 0; x < columns; ++x)
            {
                ...
                index_data[index_ptr + 1] =
                    vertex_offset + columns - 1 - x + N + 1;
                index_ptr += 2;
            }
    ...
    if (border & 0x01)
    {
        ...
        vertex_offset = level * (N + 1) * (N + 1) + (yoff + rows) * (N + 1)
                        + xoff + columns;                              // "A"
    ...
    else if (border & 0x04)
    {
        ...
        vertex_offset =
            level * (N + 1) * (N + 1) + yoff * (N + 1) + xoff; // "A"
        unsigned vertex_offset2 =
            (level - 1) * (N + 1) * (N + 1) + N * (N + 1); // "B"
    ...
    else if (border & 0x02)
    {
        ...
        vertex_offset =
            level * (N + 1) * (N + 1) + yoff * (N + 1) + xoff + columns; // "A"
    ...
    else if (border & 0x08)
    {
        ...
        vertex_offset =
            level * (N + 1) * (N + 1) + (yoff + rows) * (N + 1) + xoff; // "A"
        unsigned vertex_offset2 =
            (level - 1) * (N + 1) * (N + 1) + N * (N + 1) + N; // "B"
