...
class bsplinet
{
 ...
 public:
	...
	bsplinet(unsigned n_, const std::vector<T>& d, const std::vector<double>& tvec_)
	...
	bsplinet(unsigned n_, const std::vector<T>& d)
		: n(n_), m(unsigned(d.size())-1), cp(d), tvec(d.size()+n_+1 /* = m+n+2 */ )
	{
		static_assert(is_uniform, "bspline type and constructor use mismatch");
		unsigned k = 0;
		for ( ; k <= n; ++k) tvec[k] = 0.0;
		for ( ; k <= m; ++k) tvec[k] = double(k-n)/double(m-n+1);
		for ( ; k <= m+n+1; ++k) tvec[k] = 1.0;
		deBoor_pts.resize((n+1)*(n+2)/2);
	}
