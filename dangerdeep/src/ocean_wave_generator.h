...
#ifndef M_PI
#define M_PI \
    3.1415926535897932 // should be in math.h, but not for Windows. *sigh*
#endif
#endif

///\brief A generator class for ocean wave height data using a statistical model
///and the FFT.
template <class T> class ocean_wave_generator
{
    int N;               // grid size
    ...
    FFT_COMPLEX_TYPE *fft_in,
        *fft_in2; // can't be a vector, since the type is an array
    ...
    void allocmem()
    {
        ...
        fft_out  = nullptr;
        fft_out2 = nullptr;
        fft_in   = (FFT_COMPLEX_TYPE*) FFT_ALLOC(
            sizeof(FFT_COMPLEX_TYPE) * (N * (N / 2 + 1)));
        if (!fft_in)
        {
            freemem();
            throw std::bad_alloc();
        }
        fft_in2 = (FFT_COMPLEX_TYPE*) FFT_ALLOC(
            sizeof(FFT_COMPLEX_TYPE) * (N * (N / 2 + 1)));
        if (!fft_in2)
        {
            freemem();
  ...
  public:
    ocean_wave_generator(
        ...
        T windspeed                = T(20.0),
        T waveheight =
            T(0.0001), // fixme: compute that automatically from Lm, etc.?
        T tilesize  = T(100.0),
        T cycletime = T(10.0));
    ...
    // clearlowfreq optionally clears lower frequencies, if it is < 0, it clears
    // higher frequencies.
    ocean_wave_generator(
        const ocean_wave_generator<T>& owg, int gridsize, int clearlowfreq = 0);
    ...
    void compute_finite_normals(
        const std::vector<T>& heights, std::vector<vector3t<T>>& normals) const;
    void compute_normals(std::vector<vector3t<T>>& wavenormals) const;
    // give negative values to use default factor, positive for displacement
    // scaling
    void compute_displacements(
        const T& scalefac, std::vector<vector2t<T>>& wavedisplacements) const;
    ~ocean_wave_generator();
};

template <class T> std::complex<T> ocean_wave_generator<T>::gaussrand() const
{
    T x1, x2, w;
    do
    {
...
T ocean_wave_generator<T>::phillips(const vector2t<T>& K) const
{
    ...
    // note: a greater exponent could be used (e.g. 6) to align waves even more
    // to the wind
    T KdotW    = K * W;
    ...
    T result =
        wave_height_scale * eterm * KdotWhat
        * exp(
            -k2 * l2); // only here the term "a" (wave height scalar) is used -
                       // fixme is this dependend on the resolution? some fft
                       // scalar or so? it depends on the scale of the result!
                       // in tessendorf paper its just a numerical constant
    // fixme research how that constant correlates to fft resolution. maybe we
    // just miss some fft scale factor!
...
{
    // in comparison to the water engine by ??? here some randomization is
    // missing. there this complex number is multiplied with a random sinus value
    // that means a random phase. But it doesn't seem to change the appearance
    // much.
    // T f = sin(2*constant::PI*T(rndgen.get());
    std::complex<T> g = gaussrand(); // split in two expressions to make it
                                     // compileable under VC6
    T p = sqrt(T(0.5) * phillips(K));
    return g * p; // * f;
}

template <class T> void ocean_wave_generator<T>::compute_h0tilde()
{
...
template <class T>
std::complex<T> ocean_wave_generator<T>::h_tilde(
    const vector2t<T>& K, int kx, int ky, T time) const
{
    std::complex<T> h0_tildeK = h0tilde[ky * (N + 1) + kx];
    std::complex<T> h0_tildemKconj =
        conj(h0tilde[(N - ky) * (N + 1) + (N - kx)]);
    // all frequencies should be multiples of one base frequency (see paper), if
    // we want a looping animation
    ...
    T cxp = cos(xp);
    T sxp = sin(xp);
    return h0_tildeK * std::complex<T>(cxp, sxp)
           + h0_tildemKconj * std::complex<T>(cxp, -sxp);
}

template <class T> void ocean_wave_generator<T>::compute_htilde(T time)
{
    const T pi2 = T(2.0 * constant::PI);
    for (int y = 0; y <= N / 2; ++y)
    { // this means one sin/cos per value for every fft, so maybe this should go
      // to GPU as well!
        for (int x = 0; x < N; ++x)
        {
...
ocean_wave_generator<T>::ocean_wave_generator(
    ...
    T cycletime) :
    N(int(gridsize)),
    W(winddir.normal()), v(windspeed), wave_height_scale(waveheight),
    Lm(tilesize),
    w0(cycletime < T(0.0) ? T(0.0) : T(2.0 * constant::PI) / cycletime),
    rndgen(12345)
{
...
template <class T>
ocean_wave_generator<T>::ocean_wave_generator(
    const ocean_wave_generator<T>& owg) :
    N(owg.N),
    W(owg.W), v(owg.v), wave_height_scale(owg.wave_height_scale), Lm(owg.Lm),
    w0(owg.w0), h0tilde(owg.h0tilde), rndgen(12345)
{
...
template <class T>
ocean_wave_generator<T>::ocean_wave_generator(
    const ocean_wave_generator<T>& owg, int gridsize, int clearlowfreq) :
    N(gridsize <= owg.N ? gridsize : owg.N),
    W(owg.W), v(owg.v), wave_height_scale(owg.wave_height_scale), Lm(owg.Lm),
    w0(owg.w0), rndgen(12345)
{
    h0tilde.resize((N + 1) * (N + 1));
    ...
    for (int y = 0; y <= N; ++y)
    {
        ...
        {
            h0tilde[y * (N + 1) + x] =
                owg.h0tilde[(y + offset) * (owg.N + 1) + (x + offset)];
        }
    }
    ...
    else
    {
        ...
        for (int y = N / 2 + 1 - clearlowfreq; y <= N / 2 - 1 + clearlowfreq;
             ++y)
        {
            for (int x = N / 2 + 1 - clearlowfreq;
                 x <= N / 2 - 1 + clearlowfreq; ++x)
            {
                h0tilde[y * (N + 1) + x] = 0;
            }
        }
...
template <class T> ocean_wave_generator<T>::~ocean_wave_generator /*<T>*/ ()
{
...
template <class T> void ocean_wave_generator<T>::set_time(T time)
{
    compute_htilde(time);
}
...
{
    // this loop is a bit overhead, we could store htilde already in a
    // fft_complex array then we must transpose it, fftw has x*(N/2+1)+y, we use
    // y*N+x this overhead shouldn't matter.
    for (int y = 0; y <= N / 2; ++y)
    {
        for (int x = 0; x < N; ++x)
        {
    ...
    // so we have to add N/2 in the formulas, a term that can be seperated as
    // exponential term: exp(I*pi*(x+y)) that is equal to (-1)^(x+y), so we have
    // to adjust the sign at every second element in checkerboard form. we copy
    // the result to the output array in parallel.
...
template <class T>
void ocean_wave_generator<T>::compute_finite_normals(
    const std::vector<T>& heights, std::vector<vector3t<T>>& normals) const
{
    if (normals.size() != N * N)
        normals.resize(N * N);
    ...
    for (int y = 0; y < N; ++y)
    {
        ...
        for (int x = 0; x < N; ++x)
        {
            ...
            vector3t<T> ortho =
                (a.cross(b) + b.cross(c) + c.cross(d) + d.cross(e) + e.cross(f)
                 + f.cross(g) + g.cross(h) + h.cross(a));
            *it++ = ortho.normal();
        }
    }
}
...
template <class T>
void ocean_wave_generator<T>::compute_normals(
    std::vector<vector3t<T>>& wavenormals) const
{
    ...
    for (int y = 0; y < N; ++y)
    {
        for (int x = 0; x < N; ++x)
        {
            T s = signs[(x + y) & 1];
            wavenormals[ptr] =
                vector3t<T>(-fft_out[ptr] * s, -fft_out2[ptr] * s, T(1))
                    .normal();
            ++ptr;
        }
    }
...
template <class T>
void ocean_wave_generator<T>::compute_displacements(
    const T& scalefac, std::vector<vector2t<T>>& wavedisplacements) const
{
    for (int y = 0; y <= N / 2; ++y)
    {
        for (int x = 0; x < N; ++x)
        {
            ...
            vector2t<T> K(
                T(x - N / 2),
                T(y - N / 2)); // the factor 2*PI/Lm gets divided out later, so
    ...
    for (int y = 0; y < N; ++y)
    {
        ...
        {
            T s = signs[(x + y) & 1];
            wavedisplacements[ptr] =
                vector2t<T>(fft_out[ptr], fft_out2[ptr]) * s * scalefac;
            ++ptr;
        }
    }
}
