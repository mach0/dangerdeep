...
{
	/// Subdivide string
	static std::string next_part_of_string(const std::string& s, std::string::size_type& fromwhere)
	{
...
bool mesh::compute_tangentx(vertex_index i0, vertex_index i1, vertex_index i2)
{
	...
	float med = float((fabs(d_uv0.x) + fabs(d_uv0.y) + fabs(d_uv1.x) + fabs(d_uv1.y)) * 0.25);
	...
	if (fabsf(det) <= eps) {
		...
		// compute tangentx by multiplying the inverted uv matrix with position deltas
		float a = d_uv1.y/det;
		float b = -d_uv0.y/det;
...
mesh::mesh(std::vector<vector3f>&& positions_, std::vector<std::array<vertex_index, 3>>&& indices_,
	std::vector<vector2f>&& texcoords_, std::vector<vector3f>&& normals_, std::vector<vector3f>&& tangentsx_,
	std::vector<uint8_t>&& righthanded_)
	: positions(std::move(positions_))
	, indices(std::move(indices_))
	, normals(std::move(normals_))
	, texcoords(std::move(texcoords_))
	, tangentsx(std::move(tangentsx_))
	, righthanded(std::move(righthanded_))
	, inertia_tensor(matrix3::one())
	, volume(0.0)
	, material_id(0)
{
...
bool mesh::intersects(const mesh& other, const matrix4f& transformation_this_to_other) const
{
	// This is some brute force method, could be done faster with bounding volume trees.
...
bool mesh::is_degenerated(const vector3f& v0, const vector3f& v1, const vector3f& v2, const float eps)
{
	...
	float eps2 = eps * eps;
	if (sqrlen01 < eps2) return true;
	if (sqrlen02 < eps2) return true;
	if (sqrlen12 < eps2) return true;
	// Check that triangle area is ok
	if (delta01.cross(delta02).length() < eps) return true;
	return false;
}
...
void mesh::write_off_file(const std::string& fn) const
{
	...
	for (auto& idx : indices) {
		out << "3 " << idx[0].get_index() << " " << idx[1].get_index() << " " << idx[2].get_index() << "\n";
	}
}
...
/// Split the mesh in two parts (cut by plane), that is return split parts as new meshes
std::pair<mesh, mesh> mesh::split(const plane& split_plane) const
{
	...
	for (unsigned i = 0; i < positions.size(); ++i) {
		...
		if (dists[i] >= 0) {
			...
			if (!texcoords.empty()) part0.texcoords.push_back(texcoords[i]);
			if (!normals.empty()) part0.normals.push_back(normals[i]);
			if (!tangentsx.empty()) part0.tangentsx.push_back(tangentsx[i]);
			if (!righthanded.empty()) part0.righthanded.push_back(righthanded[i]);
		} else {
			ixtrans[i] = unsigned(part1.positions.size());
			part1.positions.push_back(positions[i]);
			if (!texcoords.empty()) part1.texcoords.push_back(texcoords[i]);
			if (!normals.empty()) part1.normals.push_back(normals[i]);
			if (!tangentsx.empty()) part1.tangentsx.push_back(tangentsx[i]);
			if (!righthanded.empty()) part0.righthanded.push_back(righthanded[i]);
		}
	}
	...
	for (unsigned i = 0; i < unsigned(indices.size()); ++i) {
		...
		if (ds[0] >= 0 && ds[1] >= 0 && ds[2] >= 0) {
			part0.indices.push_back({ixtrans[ix[0].get_index()], ixtrans[ix[1].get_index()], ixtrans[ix[2].get_index()]});
			continue;
		}
		if (ds[0] < 0 && ds[1] < 0 && ds[2] < 0) {
			part1.indices.push_back({ixtrans[ix[0].get_index()], ixtrans[ix[1].get_index()], ixtrans[ix[2].get_index()]});
			continue;
		}
		...
		for (unsigned j = 0; j < 3; ++j) {
			...
			vector3f newv = position(ix[j]) * (1 - fac) + position(ix[next[j]]) * fac;
			part0.positions.push_back(newv);
			part1.positions.push_back(newv);
			if (!texcoords.empty()) {
				vector2f newtexc = texcoords[ix[j].get_index()] * (1 - fac) + texcoords[ix[next[j]].get_index()] * fac;
				part0.texcoords.push_back(newtexc);
				part1.texcoords.push_back(newtexc);
			}
			if (!normals.empty()) {
				vector3f newnorm = (normals[ix[j].get_index()] * (1 - fac) + normals[ix[next[j]].get_index()] * fac).normal();
				part0.normals.push_back(newnorm);
				part1.normals.push_back(newnorm);
			}
			if (!tangentsx.empty()) {
				vector3f newtanx = (tangentsx[ix[j].get_index()] * (1 - fac) + tangentsx[ix[next[j]].get_index()] * fac).normal();
...
/// Smooth positions by using defined number of iterations and lambda (1.0 = full)
void mesh::smooth_positions(unsigned num_iterations, float lambda, bool keep_border)
{
...
void mesh::compute_adjacency()
{
	...
	// we use an (unordered) map with vertex pair as key value (lower vertex number first).
	// triangle and edge index are stored for the pair.
	// when we encounter the pair the second time we know the adjacency information
	std::unordered_map<uint64_t, std::pair<triangle_index, unsigned>> adjacency_data;
	...
	for (unsigned i = 0; i < nr_tri; ++i) {
		...
		if (triidx[0] != triidx[1] && triidx[1] != triidx[2] && triidx[0] != triidx[2]) {
			for (unsigned j = 0; j < 3; ++j) {
				...
				uint64_t key = (i0 < i1) ? i0 + (uint64_t(i1) << 32) : i1 + (uint64_t(i0) << 32);
				//std::cout << "Triangle " << i << " edgeindex " << j << " vertices " << i0 << " to " << i1 << " key " << key << "\n";
				auto pib = adjacency_data.insert(std::make_pair(key, std::make_pair(triangle_index(i), j)));
				if (!pib.second) {
					// vertex pair already encountered, so we can define adjacency
					auto& pairtriedge = pib.first->second;
					if (pairtriedge.first == triangle_index()) {
						// edge was encountered more than twice
						THROW(error, "mesh has more than two triangles on edge!");
					}
					triangle_adjacency[i][j] = pairtriedge.first;
					triangle_adjacency[pairtriedge.first.get_index()][pairtriedge.second] = i;
					// mark edge as used
	...
	// set vertex-triangle adjacency. vertex points to any triangle, but open edges are preferred.
	// so vertex points to triangle with open edge if there is an open edge adjacent to the vertex
	for (unsigned i = 0; i < nr_tri; ++i) {
		for (unsigned j = 0; j < 3; ++j) {
			if (triangle_adjacency[i][j] == triangle_index() ||
					vertex_triangle_adjacency[indices[i][j].get_index()] == triangle_index()) {
				vertex_triangle_adjacency[indices[i][j].get_index()] = triangle_index(i);
			}
		}
	}
}
...
{
	if (triangle_adjacency.size() != indices.size()) return false;
	for (unsigned i = 0; i < unsigned(indices.size()); ++i) {
		auto& triidx = indices[i];
		for (unsigned j = 0; j < 3; ++j) {
			if (triangle_adjacency[i][j] != triangle_index()) {
				...
				for (unsigned k = 0; k < 3; ++k) {
					if (nbtriidx[k] == i1) {
						if (nbtriidx[(k + 1) % 3] != i0) {
							//std::cout << "failed tri " << i << " edge " << j << " vertices " << i0 << " to " << i1 << " nb vertices " << nbtriidx[k] << " to " << nbtriidx[(k + 1) % 3] << "\n";
							return false;
						}
						ok = true;
					}
...
void mesh::for_all_adjacent_vertices(vertex_index vtx, const std::function<void(vertex_index)>& func)
{
	if (!has_adjacency_info()) THROW(error, "no adjacency info for 1-ring iteration");
	auto tri = get_triangle_of_vertex(vtx);
	if (tri == triangle_index()) THROW(error, "no triangle for vertex for 1-ring iteration");
	auto first_tri = tri;
...
void mesh::for_all_adjacent_triangles(vertex_index vtx, const std::function<void(triangle_index)>& func)
{
	if (!has_adjacency_info()) THROW(error, "no adjacency info for 1-ring iteration");
	auto tri = get_triangle_of_vertex(vtx);
	if (tri == triangle_index()) THROW(error, "no triangle for vertex for 1-ring iteration");
...
unsigned prepare_tri_strip(const std::vector<std::array<mesh::vertex_index, 3>>& indices, AddFunc add)
{
	...
	unsigned last_two_indices[2] = { indices[0][1].get_index(), indices[0][2].get_index() };
...
/// determine number of indices that will be needed for a triangle strip representation of this mesh.
...
/// compute for every vertex if it is on the border of the mesh. Needs correct adjacency data!
...
void mesh::load(const xml_elem& elem)
{
	...
	*this = mesh();
	...
	for (unsigned i = 0; i < nrindis; ++i) {
		...
		issi >> idx;
		if (idx >= nrverts)
			THROW(xml_error, std::string("vertex index out of range, mesh ") + name, elem.doc_name());
...
xml_elem mesh::save(xml_elem& parent) const
{
	...
	for (auto& idx : indices) {
		ossi << idx[0].get_index() << " " << idx[1].get_index() << " " << idx[2].get_index() << " ";
	}
