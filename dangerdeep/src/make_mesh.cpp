    ...
    int ti)
{
    ...
    m->normals[bv + 0] = m->normals[bv + 1] = m->normals[bv + 2] =
        m->normals[bv + 3]                  = vec(ni);
    m->tangentsx[bv + 0] = m->tangentsx[bv + 1] = m->tangentsx[bv + 2] =
        m->tangentsx[bv + 3]                    = vec(ti);
    m->righthanded[bv + 0] = m->righthanded[bv + 1] = m->righthanded[bv + 2] =
    ...
    const string& name)
{
    ...
    int cnr[24] = {3, 1, 0, 2, 7, 5, 1, 3, 6, 4, 5, 7,
                   2, 0, 4, 6, 2, 6, 7, 3, 4, 0, 1, 5};
    for (unsigned j = 0; j < 24; ++j)
    {
        int i          = cnr[j];
        m->vertices[j] = vector3f(
            (i & 1) ? w2 : -w2, (i & 2) ? l2 : -l2, (i & 4) ? h2 : -h2);
    }
    ...
    const string& name)
{
    ...
    // fixme: righthanded info is missing!!!! maybe the reason for the display
    // bugs
    ...
    for (unsigned y = 1; y < stacks; y++)
    {
        ...
        for (unsigned x = 0; x <= slices; x++)
        {
            ...
            m->texcoords[vptr] =
                vector2f(uscal * float(x) / slices, vscal * float(y) / stacks);
    ...
    const string& name)
{
    ...
    for (unsigned i = 0; i <= rsegs; ++i)
    {
        ...
        m->normals[i]                = m->normals[i + (rsegs + 1)] =
            vector3f(ca, sa, (r0 - r1) / h).normal();
        ...
        if (cap && r0 > 0)
        {
            ...
            m->texcoords[i + basecap0] =
                vector2f(0.5f + 0.5f * ca, 0.5f + 0.5f * sa);
    ...
    const string& name)
{
    ...
    for (unsigned i = 0; i <= outerrsegs; ++i)
    {
        ...
        for (unsigned j = 0; j <= innerrsegs; ++j)
        {
            ...
            float isa                             = sin(iang);
            float ica                             = cos(iang);
            m->vertices[i * (innerrsegs + 1) + j] = vector3f(
                ca * (outerr + ica * innerr),
                sa * (outerr + ica * innerr),
                isa * innerr);
            m->texcoords[i * (innerrsegs + 1) + j] =
                vector2f(uscal * j / innerrsegs, vscal * i / outerrsegs);
            m->normals[i * (innerrsegs + 1) + j] =
                vector3f(ca * ica, sa * ica, isa);
    ...
    for (unsigned i = 0; i < outerrsegs; ++i)
    {
        for (unsigned j = 0; j < innerrsegs; ++j)
        {
            ...
            m->indices[(i * innerrsegs + j) * 6 + 2] =
                m->indices[(i * innerrsegs + j) * 6 + 3] =
                    i * (innerrsegs + 1) + j + 1;
            m->indices[(i * innerrsegs + j) * 6 + 1] =
                m->indices[(i * innerrsegs + j) * 6 + 4] =
                    (i + 1) * (innerrsegs + 1) + j;
            m->indices[(i * innerrsegs + j) * 6 + 5] =
                (i + 1) * (innerrsegs + 1) + j + 1;
        }
    }
    return m;
}
