...
class vector2t
{
 public:
	...
	template<typename E> explicit vector2t(const vector2t<E>& other) : x(D(other.x)), y(D(other.y)) {}
	...
	vector2t<D> operator* (D scalar) const { return vector2t(x * scalar, y * scalar); }
	vector2t<D> operator/ (D scalar) const { return vector2t(x / scalar, y / scalar); }
	...
	vector2t<D> operator& (D mask) const { return vector2t(x & mask, y & mask); }	// won't work with float.
	vector2t<D>& operator+= (const vector2t<D>& other) { x += other.x; y += other.y; return *this; }
	vector2t<D>& operator-= (const vector2t<D>& other) { x -= other.x; y -= other.y; return *this; }
	vector2t<D>& operator*= (D s) { x *= s; y *= s; return *this; }
	vector2t<D> min(const vector2t<D>& other) const { return vector2t(std::min(x, other.x), std::min(y, other.y)); }
	vector2t<D> max(const vector2t<D>& other) const { return vector2t(std::max(x, other.x), std::max(y, other.y)); }
	...
	template<typename D2> friend std::ostream& operator<< ( std::ostream& os, const vector2t<D2>& v );
	template<typename E> void assign(const vector2t<E>& other) { x = D(other.x); y = D(other.y); }
	...
	vector2t<D> frac() const { return vector2t(D(x - ::floor(x)), D(y - ::floor(y))); }
	vector2t<D> mod(D v) const { return *this - (*this / v).floor() * v; } // negative values are problematic for fmod. So we use own formula fmod(a, b) = a - floor(a / b) * b
	vector2t<D> abs() const { return vector2t<D>(x < D(0) ? -x : x, y < D(0) ? -y : y); }
...
template<typename D2> inline vector2t<D2> operator* (D2 scalar, const vector2t<D2>& v)
{
	return v * scalar;
}

template<typename D2> std::ostream& operator<< (std::ostream& os, const vector2t<D2>& v)
{
	os << "x=" << v.x << "; y=" << v.y;
	return os;
}
