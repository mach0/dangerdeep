...
camera camera::create_look_at(const vector3& pos, const vector3& look_at, const vector3& up,
			      double fovx, double aspectratio, double nearz, double farz)
{
	return create_look_dir(pos, look_at - pos, up, fovx, aspectratio, nearz, farz);
}



camera camera::create_look_dir(const vector3& pos, const vector3& look_dir, const vector3& up,
			       double fovx, double aspectratio, double nearz, double farz)
{
	camera cam;
...
void camera::set_position_and_look_direction(const vector3& pos, const vector3& look_dir, const vector3& up)
{
	position = pos;
	...
	// Build orthogonal system. Since side and look dir are already orthogonal and
	// normalized, the resulting up direction will be as well.
	const vector3 up_direction = side_direction.cross(look_direction);	// X cross Z, so not -look_dir here
...
{
	// Compute transformation matrix - that is inverse of system combined of the stored vectors.
	// We build it by transposing the rotation part and multiplying with position.
	// X is side, Y is up, and Z is -look.
	...
	return matrix4( side_direction.x,  side_direction.y,  side_direction.z,  -p.x,
			up_direction.x,    up_direction.y,    up_direction.z,    -p.y,
			nlook_direction.x, nlook_direction.y, nlook_direction.z, -p.z,
			0.0,               0.0,               0.0,               1.0);
}
...
{
	return (is_neutral ? matrix4::one() : matrix4::frustum_fovx(field_of_view_x, aspect_ratio, near_z, far_z));
}
...
{
	draw::instance().wire_cube(cam, get_pmv_matrix().inverse(), 1.0f, color::white());
	...
	texquadarray_dummy(1, 1, 1, 3, data_type::u8, false)
{
	...
	shader vsquadtex(
		...
		"const vec2 positions[4] = { vec2(0.0, 0.0), vec2(1.0, 0.0), vec2(1.0, 1.0), vec2(0.0, 1.0) };\n"
		"const vec2 texcoords[4] = { vec2(0.0, 0.0), vec2(1.0, 0.0), vec2(1.0, 1.0), vec2(0.0, 1.0) };\n"
		"layout(std140, binding = 0) uniform in_uni { vec4 p_off_scal; vec4 t_off_scal; uint layer; } inp;\n"
		"out vec2 texcoord;\n"
		"void main() {\n"
		"texcoord = texcoords[gl_VertexID] * inp.t_off_scal.zw + inp.t_off_scal.xy;\n"
		"gl_Position = vec4(positions[gl_VertexID] * inp.p_off_scal.zw + inp.p_off_scal.xy, 0.0, 1.0);\n"
		"}\n",
		shader::type::vertex, true);
	shader fstex(
		"#version 430 core\n"
		"in vec2 texcoord;\n"
	...
	shader fstexarray(
		...
		"layout(std140, binding = 0) uniform in_uni { vec4 p_off_scal; vec4 t_off_scal; uint layer; } inp;\n"
		"out vec4 frag_color;\n"
		"void main() { frag_color = vec4(texture(tex, vec3(texcoord, inp.layer)).xyz, 1.0); }\n",
		shader::type::fragment, true);
	...
	shader vslines(
		"#version 430 core\n"
		"layout(std140, binding = 0, row_major) uniform in_uni { mat4 pmv; vec4 col; };\n"
	...
	// not always sensible: rc_coltris.set_face_render_side(face_render_side::both);
	rc_coltris.init();
}
...
{
	// we need to transform coordinates and widths/heights with a 2D scaling factor. Y-Coordinates are also negative, so we have a scale and offset for positions.
...
void draw::quad(const texture_array& tex, unsigned layer, const vector2i& pos, const vector2i& size)
{
	// we need to transform coordinates and widths/heights with a 2D scaling factor. Y-Coordinates are also negative, so we have a scale and offset for positions.
	vector2f p = SYS().translate_2D_coordinates(pos);
	vector2f s = SYS().translate_2D_size(size);
...
void draw::quad_n(const texture_array& tex, unsigned layer, const vector2i& pos, const vector2i& size)
{
	// we need to transform coordinates and widths/heights with a 2D scaling factor. Y-Coordinates are also negative, so we have a scale and offset for positions.
	vector2f p = SYS().translate_2D_coordinates(pos);
	vector2f s = SYS().translate_2D_size(size);
...
void draw::lines(const camera& cam, const std::vector<vector3f>& positions, color col)
{
	...
	rc_lines.use();
	rc_lines.draw_primitives(primitive_type::lines, 0, unsigned(positions.size()) & ~1);
}



void draw::colored_triangles(const camera& cam, const std::vector<vector3f>& positions, const std::vector<color>& colors)
{
	...
	rc_coltris.use();
	rc_coltris.draw_primitives(primitive_type::triangles, 0, unsigned(positions.size()));
}



void draw::line_strip(const camera& cam, const std::vector<vector3f>& positions, color col)
{
	if (positions.size() < 2) return;
	line_udata data;
	data.pmv = cam.get_pmv_matrix();
	...
	vbo_lines.update(positions, buffer::usage_type::stream_draw);
	rc_lines.use();
	rc_lines.draw_primitives(primitive_type::line_strip, 0, unsigned(positions.size()));
}
...
void draw::wire_cube(const camera& cam, const matrix4f& cs, float hel, color col)
{
	std::array<vector3f, 8> vertices;
	for (unsigned i = 0; i < 8; ++i) {
		vertices[i] = cs.mul4vec3(vector3((i & 1) ? hel : -hel, (i & 2) ? hel : -hel, (i & 4) ? hel : -hel));
	}
	std::vector<vector3f> positions(24);
	static const unsigned idx[24] = { 0, 1, 1, 3, 3, 2, 2, 0, 0, 4, 1, 5, 2, 6, 3, 7, 4, 5, 5, 7, 7, 6, 6, 4 };
...
void draw::debug_cube(const camera& cam, const matrix4f& cs, float hel)
{
	...
	static const unsigned idx[36] = { 0, 1, 2, 2, 1, 3,
					  1, 5, 3, 3, 5, 7,
					  2, 3, 6, 6, 3, 7,
					  5, 4, 7, 7, 4, 6,
					  4, 0, 6, 6, 0, 2,
					  4, 5, 0, 0, 5, 1 };
	static const color cols[6] = { color(255, 0, 0, 64), color(0, 255, 0, 64), color(0, 0, 255, 64),
				       color(128, 0, 0, 64), color(0, 128, 0, 64), color(0, 0, 128, 64) };
	...
	: current_camera_index(0)
{
	...
	ld.position = vector4f(get_current_camera().get_transformation().inverse() * lightpos);
...
void scene::set_current_camera_position_and_look_at(const vector3& pos, const vector3& look_at, const vector3& up)
{
...
{
	light_data ldc = ld;
	// light position is in world space. We need to transform it to camera space,
	// so inverse modelview matrix converts it from camera space to object space
	// later in shader.
	lightpos = vector4(ld.position);
	ldc.position = vector4f(get_current_camera().get_transformation() * lightpos);
...
void scene::set_clip_plane(const plane& clipplane_)
{
	...
	clipplane_data cd;
	cd.clipplane = vector4f(get_current_camera().get_transformation() * clipplane.N.xyz0());
...
compute_FFT::compute_FFT(texture& workspace_, bool forward, bool use_half_float)
 :	workspace(workspace_), local_size(16)
{
	if (workspace.get_nr_of_channels() != 2 || workspace.get_width() != workspace.get_height())
	...
	temp_workspace = texture(fft_size, fft_size, 2, use_half_float ? data_type::f16 : data_type::f32);
	...
	// note! std140 alignment will align array of vec2 to vec4! For std430 alignment this is hopefully better...
	oss << "#version 430 core\n"
		<< "layout(binding = 0, " << texfmt << ") readonly uniform image2D input_values;\n"
		<< "layout(binding = 1, " << texfmt << ") writeonly uniform image2D output_values;\n"
		<< "layout(local_size_x = " << local_size << ", local_size_y = " << local_size << ") in;\n"
		<< "layout(std430, binding = 0) readonly buffer index_data { uint indices[" << fft_size << " * 2]; } id;\n"
		<< "layout(std430, binding = 1) readonly buffer factor_data { vec2 factors[" << fft_size << "]; } fc;\n"
		<< "vec2 complex_mul(vec2 x, vec2 y) { return vec2(x.x * y.x - x.y * y.y, x.y * y.x + x.x * y.y); }\n"
		<< "void main() {\n"
		<< "ivec2 coords = ivec2(gl_GlobalInvocationID.x, gl_GlobalInvocationID.y);\n"; // Read the image data with input indices
	std::string code_begin = oss.str();
	// Now row/column specific part
	std::string code_rows =
		"ivec2 coords_a = ivec2(id.indices[2 * gl_GlobalInvocationID.x + 0], gl_GlobalInvocationID.y);\n"
		"ivec2 coords_b = ivec2(id.indices[2 * gl_GlobalInvocationID.x + 1], gl_GlobalInvocationID.y);\n"
		"vec2 factor = fc.factors[gl_GlobalInvocationID.x];\n";
	std::string code_columns =
		"ivec2 coords_a = ivec2(gl_GlobalInvocationID.x, id.indices[2 * gl_GlobalInvocationID.y + 0]);\n"
		"ivec2 coords_b = ivec2(gl_GlobalInvocationID.x, id.indices[2 * gl_GlobalInvocationID.y + 1]);\n"
	...
	cc_fft_columns.add(shader(code_begin + code_columns + code_common + code_end, shader::type::compute, true));
	cc_fft_columns.set_compute_size(fft_size / local_size, fft_size / local_size, 1);
	...
	cc_fft_rows.add(shader(code_begin + code_rows + code_common + code_end, shader::type::compute, true));
	cc_fft_rows.set_compute_size(fft_size / local_size, fft_size / local_size, 1);
	...
	cc_fft_columns_LL.add(shader(code_begin + code_columns + code_common + code_output_transform + code_end, shader::type::compute, true));
	cc_fft_columns_LL.set_compute_size(fft_size / local_size, fft_size / local_size, 1);
	...
	cc_fft_rows_LL.add(shader(code_begin + code_rows + code_common + code_output_transform + code_end, shader::type::compute, true));
	cc_fft_rows.set_compute_size(fft_size / local_size, fft_size / local_size, 1);
}
...
void compute_FFT::compute()
{
	...
	// Geforce 610: ~100ms for 1024 fft. Now 67ms for 1024 with half float. We need at least 256 fft, should be <4ms and fast enough
	// With 512 fft frame <= 16ms.
	// Computation is memory bound. Memory access order is not the problem.
	// It seems we can't go faster with compute shaders than this, nor with fragment shaders.
	// Maybe higher radix kernels can reduce memory accesses (needs fewer output writes, but more index reads)
	// Memory bandwidth could be decreased by using half float textures. (On GT580 from 400ms to 350ms, but there drawing already takes much bandwidth?!) So it helps
	// We can give output scaling (to equalize results) on last run.
	// If we compute inverse transform (frequencies to real coordinates) we don't need the upper half
	// of frequencies (too high), this could save some computation steps.
...
std::vector<unsigned> compute_FFT::compute_reversed_bits_indices(unsigned exponent)
{
...
void compute_FFT::compute_indices_and_factors(bool forward)
{
	...
	// We can store it as 2D table for the shader, so we won't need to rebind uniform buffers,
	// on the other hand we need to give an uniform number which buffer part to use, so just
	// binding a different buffer may be faster.
	// for the first level we need to read from reversed bit indices!
	// but be careful, u values are not reversed...
	// Maybe indices can be computed for levels 1... with clever bit arithmetic in shader and that would be faster?
	vector2 c(-1.0, 0.0);
	...
	for (unsigned level = 0; level < fft_size_log2; ++level) {
		...
		for (unsigned j = 0; j < l1; ++j) {
			...
			for (unsigned i0 = j; i0 < fft_size; i0 += l2) {
				...
				// This means every output value is a linear combination of input values.
				// We need to store the two indices of the computation source and the two factors.
				unsigned idx0 = i0;
				unsigned idx1 = i1;
				if (level == 0) {
		...
		factor_buffers[level].init(buffer::usage_type::static_draw, fft_factors);
	}
}
...
program gpu::make(const shader_source_helper& ssh)
{
	...
	return program(shader(vs, shader::type::vertex, true), shader(fs, shader::type::fragment, true));
}



shader_source_helper gpu::get_noise_shader_data(unsigned resolution, unsigned base_factor, unsigned nr_of_levels, unsigned tex_unit, unsigned offset_slot, float amplitude_scale)
{
	shader_source_helper ssh;
	std::ostringstream oss;
	oss <<
		"layout(binding = " << tex_unit << ") uniform sampler2D tex_noise;\n"
		"layout(std140, binding = " << offset_slot << ") uniform inp { vec4 offsets[" << nr_of_levels << "]; }; \n" // only vec2, but alignment...
	...
	for (unsigned level = 0; level < nr_of_levels; ++level) {
		oss << "texture(tex_noise, offsets[" << level << "].xy + coord * " << float(base_factor << level) / resolution << ").x * " << amplitude;
...
namespace
{
	auto add_ubo_def(basic_shader_uniform_location bsul, const char* name, const char* add_defs = "")
	{
		return std::string("layout(std140, binding = ") + helper::str(unsigned(bsul)) + add_defs + ") uniform " + name + " {\n";
	}
}

std::string transform_data::get_definition()
{
	return add_ubo_def(basic_shader_uniform_location::transform, "transform_data", ", row_major") + // declaration of row_major is essential!
...
{
	return add_ubo_def(basic_shader_uniform_location::clipplane, "clipplane_data") +
		"  vec4 clipplane; \n"
		"}; \n";
}
...
shader_source_helper gpu::generate_basic_shader_source(basic_shader_feature bsf)
{
	...
	auto add_v2f_param = [&result](const char* type_and_name, const char* qualifier = "") {
		result.vertex_defs += qualifier + std::string(" out ") + type_and_name + ";\n";
		result.fragment_defs += qualifier + std::string(" in ") + type_and_name + ";\n";
	};
	auto use = [&bsf](basic_shader_feature b) {
		return (int(bsf) & int(b)) != 0;
	};

	// fixme mirrorclip: just clip with some plane? modelviewmatrix has determinate -1, a problem for rendering??
	// just give the mirrored matrix? define plane in what space? projection space?! does that work?!

	// With lighting as option we can use the shader then for basic drawing like 2D widgets.

	auto add_vattr_def = [](basic_shader_attribute_location bsal, const char* type_and_name) {
		return std::string("layout(location = ") + helper::str(unsigned(bsal)) + ") in " + type_and_name + "; \n";
	};
	auto add_tex_def = [](basic_shader_sampler_location bssl, const char* name) {
		return std::string("layout(binding = ") + helper::str(unsigned(bssl)) + ") uniform sampler2D " + name + "; \n";
	};

	// fragment shader always gives a color with alpha value.
	result.fragment_defs += "out vec4 frag_color; \n";

	// Position and transformation - Vertex shader always gets a position and transforms it.
	result.vertex_defs +=
		add_vattr_def(basic_shader_attribute_location::position, "vec3 position") +
		transform_data::get_definition();
	result.vertex_code +=
		"gl_Position = transform.projection_modelview * vec4(position, 1.0); \n";
	...
	if (use(basic_shader_feature::clipplane)) {
		...
		result.vertex_code += "clipplane_distance = dot(clipplane.xyz, position) + clipplane.w; \n"; // same function as for caustics...
	...
	if (use(basic_shader_feature::lighting) || (!use(basic_shader_feature::colormap) && !use(basic_shader_feature::vertex_color))) {
		result.fragment_defs += material_data::get_definition();
	}

	// Lighting (optional)
	if (use(basic_shader_feature::lighting)) {
		add_v2f_param("vec3 lightdir", "noperspective");	// directions should be interpolated noperspective!
		...
		result.vertex_defs += add_vattr_def(basic_shader_attribute_location::normal, "vec3 vnormal");
		result.vertex_code +=
			// compute direction to light in object space (L)
			// light.position.w is 0 or 1, 0 for directional light, 1 for point light
			"const vec3 lightpos_obj = vec3(transform.modelview_inverse * light.position); \n"
			"const vec3 lightdir_obj = normalize(lightpos_obj - position * light.position.w); \n"
			// compute direction to viewer (E) in object space (mvinv*(0,0,0,1) - inputpos)
			"const vec3 viewerdir_obj = normalize(vec3(transform.modelview_inverse[3]) - position); \n"
			// compute halfangle vector (H = ||L+E||)
			"const vec3 halfangle_obj = normalize(viewerdir_obj + lightdir_obj); \n";
	...
	// texcoords
	if (use(basic_shader_feature::colormap) || use(basic_shader_feature::normalmap)
			|| use(basic_shader_feature::specularmap) || use(basic_shader_feature::underwater)) {
		add_v2f_param("vec2 texcoord0");
		result.vertex_defs += add_vattr_def(basic_shader_attribute_location::texcoord, "vec2 texcoord");
		result.vertex_code += "texcoord0 = texcoord; \n";
	}

	// diffuse color
	if (use(basic_shader_feature::colormap)) {
		result.fragment_defs += add_tex_def(basic_shader_sampler_location::color_map, "tex_color");	// 3 channels
		result.fragment_code +=
		...
		result.vertex_defs += add_vattr_def(basic_shader_attribute_location::color, "vec4 vcolor");
	...
	if (use(basic_shader_feature::normalmap)) {
		...
		result.vertex_defs +=
			add_vattr_def(basic_shader_attribute_location::tangentx, "vec3 tangentx") +
			add_vattr_def(basic_shader_attribute_location::righthanded, "int righthanded"); // stored as u8, no space wasted. type bool isn't accepted by OpenGL.
		result.vertex_code +=
			// compute tangenty, tangentz
			"const vec3 tangenty = cross(vnormal, tangentx) * (righthanded != 0 ? 1.0 : -1.0); \n"
		...
		result.fragment_defs += add_tex_def(basic_shader_sampler_location::normal_map, "tex_normal");	// 3 channels
		result.fragment_code +=
			// get and normalize normal vector from texmap
			"const vec3 N = normalize(vec3(texture(tex_normal, texcoord0)) * 2.0 - 1.0); \n";
	} else {
	...
	// Now we have a vec3 material_color and need to output a vec3 combined_color depending on lighting configuration.
	// The ambient is a factor of the light source defining a minimum brightness.
	// So ambient is computed together with brightness from light source as factor of diffuse color.
	// Specular color is a material property and can have a different color than the diffuse color.
	if (use(basic_shader_feature::lighting)) {
		if (use(basic_shader_feature::specularmap)) {
			result.fragment_defs += add_tex_def(basic_shader_sampler_location::specular_map, "tex_specular");	// 1 channel
			result.fragment_code += "const float specular_factor = texture(tex_specular, texcoord0).x; \n";
		} else {
			result.fragment_code += "const float specular_factor = 1.0; \n";
		}
		result.fragment_code +=
			"const float brightness_light = clamp(dot(L, N), 0.0, 1.0); \n"
			"const float brightness_material = mix(brightness_light, 1.0, light_ambient_factor); \n"
			"const vec3 diffuse_color = material_color * brightness_material; \n"	// also contains ambient part
			"const vec3 specular_color = material.specular_color * (pow(clamp(dot(H, N), 0.0, 1.0), material.shininess) * specular_factor); \n"
			"const vec3 combined_color = (diffuse_color + specular_color) * light_color; \n";
	}
	...
	if (use(basic_shader_feature::underwater) || use(basic_shader_feature::fog)) {
		...
		result.vertex_code += "fog_frag_coord = gl_Position.z;\n";	// could also take distance from camera (without projection matrix!)
		// our fog is exponential fog.
		result.fragment_code += "const float fog_factor = clamp(exp2(-fog.density * fog_frag_coord), 0.0, 1.0); \n";
	}
	if (use(basic_shader_feature::underwater)) {
		...
		result.vertex_defs +=
			...
			"float calculate_caustic_coords(const vec3 pos, const vec4 plane) { return dot(pos, plane.xyz) + plane.w; } \n"; // this a function to compute distance to a plane
		result.fragment_defs += add_tex_def(basic_shader_sampler_location::caustics_map, "tex_caustic");	// 1 channel
		result.vertex_code += "caustic_texcoord = vec2(calculate_caustic_coords(position, plane_s), calculate_caustic_coords(position, plane_t)); \n";
		result.fragment_code +=
			"const vec3 combined_caustic_color = combined_color * max(texture(tex_caustic, caustic_texcoord).x *2.0, 0.5); \n"
			"const vec3 fog_color = fog.color * clamp(150.0 / fog_frag_coord, 0.0, 1.0); \n"
			"frag_color = vec4(mix(fog_color, combined_caustic_color.xyz, fog_factor), alpha); \n";
	} else if (use(basic_shader_feature::fog)) {
		// normal fog code (linear fog)
		result.fragment_code +=
			"frag_color = vec4(mix(fog.color, combined_color.xyz, fog_factor), alpha); \n";
	} else {
