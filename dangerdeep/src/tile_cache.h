...
template <class T> class tile_cache
{
  public:
    ...
    typedef typename std::map<vector2i, tile<T>, coord_compare>::iterator
        tile_list_iterator;

    /* Constructs a tile_cache object and generates the morton lookup tables
     *
     *
     * tile_folder: The absolute path to the folder the tiles reside in. Needs a
     * file separator at the end. overall_rows: number of rows of the tiled
     * image overall_cols: number of rows of the tiled image tile_size: edge
     * length of one tile (tiles are square) slots: capacity of the cache. zero
     * means infinite expire: number of millisecs after a tile expire when it
    ...
    tile_cache() = default;
    ;
    /* Returns a value from the corresponding tile. If the tile isn't in the
     * cache it's added to it.
     *
     * coord: should be clear. Note that it takes global coordinates, no tile
     * local coordinates!
     */
  ...
  protected:
    ...
    /* computes the bottom left corner of correspondig tile to the given global
     * coordinates */
...
template <class T> T tile_cache<T>::get_value(vector2i coord)
{
    ...
    else
    {
        ...
        std::pair<tile_list_iterator, bool> p = tile_list.insert(
            std::pair<vector2i, tile<T>>(tile_coord, tile<T>()));
        p.first->second.load(
            filename.str().c_str(), tile_coord, configuration.tile_size);
