...
public:
	...
	std::vector<uint8_t> receive_packet(IPaddress* ip = 0);
	void send_message(const std::string& msg);
	std::string receive_message(IPaddress* ip = 0);
	
	// create a connection on a local port (0 means any free port)
	network_connection(uint16_t local_port = 0);
	// create a connection and connect to a server
	network_connection(IPaddress serverip);
	// create a connection and connect to a server
	network_connection(const std::string& servername, uint16_t server_port);
	void bind(IPaddress ip);
	void bind(const std::string& servername, uint16_t server_port);
	void unbind();
	~network_connection();
