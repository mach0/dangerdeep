...
auch wind-faktor (f�r bewegung), der ist am stamm 0 und wird immer gr��er bis zu
den bl�ttern.
^^^<<<<<<<<<<<<<<<<<<<

bl�tter einzeln, oder eben textur wie mini-ast mit bl�ttern,
diesen in einem quad, oder mit leichter beugung in 2-3

orientierung der bl�tter-quads nach oben besser,
leichte varianz!

imposter f�r sp�tere.
...
std::unique_ptr<model> tree_generator::generate() const
{
    ...
    auto* mat2 = new model::material_glsl(
        "bark", "reliefmapping.vshader", "reliefmapping.fshader");
    mat2->nrtex       = 2;
...
#endif
    auto* mat = new model::material_glsl(
        "leaf", "treeleaves.vshader", "treeleaves.fshader");
    mat->nrtex       = 1;
    ...
    bool generate_leaves) const
{
    if (segs < 3 || bend_segs < 1)
        THROW(error, "treegenerator::generate_log");
    const unsigned nr_verts = (segs + 1) * (bend_segs + 1) + 1;
    const unsigned nr_indis =
        (2 * (segs + 1) + 2) * (bend_segs + 1) - (msh.indices.empty() ? 1 : 0);
    ...
    {
        vector3f a = root + axis * (float(i) / 3 * length)
                     + xaxis * ((rnd() - 0.5) * bend_factor * length)
                     + yaxis * ((rnd() - 0.5) * bend_factor * length);
        cp.push_back(a);
    }
    ...
    if (rnd(4) == 0 && lvl < 3)
    {
        ...
        for (unsigned i = 0; i < c; ++i)
        {
            ...
            generate_tree(
                msh, mshleaves, lvl + 2, bsp.value(0.3 + 0.4 * rnd()), axisc);
        }
    }
    if (generate_leaves)
    {
        ...
        for (unsigned i = 0; i < nrl; ++i)
        {
            ...
            ++vidx;
            mshleaves.vertices[vidx] = p + lxaxis * (2 + rnd() * 0.25) * x
                                       + lyaxis * (y * (rnd() - 0.5) * 0.25);
            mshleaves.normals[vidx]   = axis;
