...
/* how its done:
   ...
   compute signal strength for four frequency bands
   (0-1kHz,1-3kHz,3-6kHz,6-7kHz). compute direction of signal. compute sonar
   signal sensitivity (depends on sonar type - specific sensitivity - and of
     speed of receiver. Higher speeds decrease sensitivity, much more effect
   than speed of targets for target noise strength). depending on angle of ghg
   apparaturs reduce signal strength of all signals (cos(angle) between
   direction and ghg angle = normal vector from ghg angle). (NOTE: do this ^
   with new reduction function!) handle background noise of own sub/ship compute
   and add background noise (ambient noise) subtract own noise + sensitivity,
   rest is signal strength make signal strengths discrete, depending on
   frequency, lower freqs -> larger steps !NO! discrete steps not in dB but
   depending on angle! lower freq -> bigger steps !NO! discretize ALL signals
   only to dB, independent of frequency!!! sum of strenghts is resulting noise
   strength, feed to user's headphones or to sonar operator simulator weighting
   of various frequency strengths depends on human perception and width of freq.
   bands 1-3 kHz *0.9, 0-1 kHz *1, 3-6 kHz *0.7 6-7 kHz *0.5, total strength is
   weighted sum div. weight sum of used bands.
   => noise of ships must be stored in distributed frequencies or we need a
   online bandpass filter to weaken higher frequencies blind spots of GHG etc
   need to be simulated, BG is blind to aft, GHG to front/aft, KDB to ? NOTE:
   compute that with strength reduction formula!
   ...
   s_i(t_i) = s_j(t_j) for i,j in 0,...11 for 12 hydrophones, and t_n = t_0 + n
   * delta_t delta_t can be computed, it is the the time a signal needs to
   travel in water to cover the distance between the two hydrophones. If the
   electrical delay caused by the strip lines of the strip line array inside the
   GHG match delta_t, then all signals are overlayed with the same time factor:
   strip line output = sum over i of s_i(t_i ) = sum over i of s_i(t_0 + i *
   delta_t) We know that a sum of signals gives the signal with maximum
   amplitude when there is no phase shift between them, e.g. sum_i=0..3
   sin(x+i*m) for any small m is maximal when m is zero. The larger the absolute
   value of m is the weaker the signal gets. Thus, the strip line output is
   maximized, when the delay of each signal in water equals the electrical
   delay. (Note that the formulas above match a situation where a wave comes
   from the bow and the strip line array's membranes are perpendicular to the
   sound wave, thus the GHG points also to the bow. But it even works when the
   hydrophones are arranged in a line from bow to aft and the sound wave comes
   directly from starboard direction - in that case the sound hits all
   hydrophones at the same time, all signals go to one strip line and thus the
   output is maximized. When the strip line array is turned away then the
   hydrophone outputs are distributed over multiple lines, causing delay and
   phase shift of the summands of the signal, thus leading to a weaker total
   output.) Proof: the hydrophones are arranged in an array with roughly 2.2m
   length. To travel that distance the sound takes 1.5ms. The strip line array
   has 100 strips with 17�s delay between each strip given a total maximum delay
   of 1.7ms. This fits. When the strip line array is not facing the signal
   perpendicular, the electrical delay and the sound delay in water differs,
   leading to a phase shift of signals in any direction, weakening the total
   signal output. This is modelled by the fall-off function that is a function
   of the angle between the GHG apparatus angle and the incoming signal's angle
   and also of the frequency of the signal. We only need to compute an
   historically accurate fall-off function. This is done by simulating the phase
   shifts and thus a real GHG as close as possible. (at the moment the function
   compute_signal_strength_GHG() does this).
...
// centigrade and f is frequency in kHz, thus take f = real_f * 10^-3, and f^2 =
// real_f^2 * 10^-6 and so (2.1 * 10^-16 * (T-38)^2 + 1.3*10^-13) if we
// assume 10.38� for water in open ocean this gives (2.1 * 10^-16 * 10^6
// + 1.3*10^-13) = (2.1*10^-10 + 1.3*10^-13) = 2101.3 * 10^-13, thus absorption
// in dB/m is 2101.3 * 10^-13 * real_f^2 this gives for 100Hz: 2.1013e-6,
...
// 100dB + 1dB/ m/s = 104.1 dB propagation loss is -20*log10(distance) = -85.4
// dB absorption = 10*log(db^104.1 - db^85.4) / 10sm = 104.04 dB / 18520m =
// 0.00562

const double noise_signature::typical_noise_signature
    [NR_OF_SHIP_CLASSES][noise::NR_OF_FREQUENCY_BANDS] = {
...
noise noise::compute_ambient_noise_strength(double seastate)
{
    ...
    {
        result.frequencies[band] = pow(
            dB_base, background_noise[band] + seastate_factor[band] * seastate);
    }
    return result;
}
    ...
    bool caviation) const
{
    ...
    for (unsigned band = 0; band < noise::NR_OF_FREQUENCY_BANDS; ++band)
    {
        ...
        // I_prop = I / R^2     and   L = 10 * log_10 (I / I_0), here  L = 10 *
        // log_10 (I) so  L_prop = 10 * log_10 (I / R^2) = 10 * log_10 (I) - 10
        // * log_10 (R^2)
        //            = L - 20 * log_10 (R)
        if (distance < 1)
            distance = 1;
...
shipclass noise::determine_shipclass() const
{
    ...
    {
        const double* typical_signature =
            noise_signature::typical_noise_signature[k];
        double class_strength_normalizer = typical_signature[0];
        for (unsigned i = 1; i < NR_OF_FREQUENCY_BANDS; ++i)
        {
            class_strength_normalizer =
                std::max(class_strength_normalizer, typical_signature[i]);
        }
    ...
    // fixme: if error is above a certain level, type can't be determined, set
    // to none...
    return myclass;
}
    ...
    angle apparatus_angle)
{
    ...
    // GHG constants
    static const double strip_delay      = 0.000017;       // 17�s
    ...
    */
    ...
    static const double height_of_all_contacts =
        distance_contact * (nr_hydrophones - 1);
    ...
    for (unsigned i = 0; i < nr_hydrophones; ++i)
    {
        ...
        angle fov_center = hydrophone_fov_center_first
                           + hydrophone_fov_center_delta * double(i);
        if (app_on_port)
        ...
        double y = (height_of_all_contacts * 0.5 - distance_contact * i)
                   * apparatus_angle.cos();
        int y_line = int(floor(y + nr_of_strips * 0.5));
        // printf("y=%f yp=%f ryp=%f y_line=%i\n", y, y + nr_of_strips*0.5,
        // floor(y + nr_of_strips*0.5), y_line);
        ...
        //       delay_i, delta_t_signal, i, time_scale_fac, (delay_i +
        //       delta_t_signal * i));
    ...
    // numbers, but can be seen also with infinite small strips. A rounding
    // error? a result of the sidelobe phenomenon?! ghg phenomenon, disappears
    // with smaller strip lines.

    // Note2: a signal seems to cause a second signal with 90� offset...
    // sidelobe?! ghsot signal. appears only with higher frequencies, ghg
    // phenomenon. maybe the arrangement of the hydrophones, their fov and
    // listening direction is not fully correct, causing or amplifying these
    // phenomenons, fixme! each hydrophone must listen to a certain direction,
    // if all listen to 90� then the output is unusable, showing peaks in
    // direction of each hydrophone. If each hydrophone has a fov < 180� the
    // output is also much worse, causing many ghost signals and so on. The same
    // is true for fov > 180�.
    ...
    // signalstrength/max_strength, apparatus_angle.value());
    // now compute percentage of max. strength
    // fixme: try this: don't divide here!!! doesn't help much, but a bit
    // though, seems to lessen jitter...
