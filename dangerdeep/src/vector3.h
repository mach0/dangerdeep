...
class vector3t
{
	...
	template<typename E> vector3t(const vector3t<E>& other) : x(D(other.x)), y(D(other.y)), z(D(other.z)) {}
	...
	vector3t<D> orthogonal(const vector3t<D>& other) const { return cross(other); }
	vector3t<D> operator* (D scalar) const { return vector3t(x * scalar, y * scalar, z * scalar); }
	vector3t<D> operator/ (D scalar) const { return vector3t(x / scalar, y / scalar, z / scalar); }
	...
	vector3t<D>& operator-= (const vector3t<D>& other) { x -= other.x; y -= other.y; z -= other.z; return *this; }
	vector3t<D>& operator*= (D s) { x *= s; y *= s; z *= s; return *this; }
	...
	vector3t<D> abs() const { return vector3t(x < D(0) ? -x : x, y < D(0) ? -y : y, z < D(0) ? -z : z); }
	...
	vector3t<D> cross(const vector3t<D>& other) const { return vector3t(y * other.z - z * other.y, z * other.x - x * other.z, x * other.y - y * other.x ); }
	...
	template<typename D2> friend std::ostream& operator<< ( std::ostream& os, const vector3t<D2>& v );
	template<typename E> void assign(const vector3t<E>& other) { x = D(other.x); y = D(other.y); z = D(other.z); }
	...
	constexpr vector3t<D>(axis a) : vector3t<D>() {
		switch (a) {
		case axis::x:		x = D(1); break;
		case axis::y:		y = D(1); break;
		case axis::z:		z = D(1); break;
		case axis::neg_x:	x = D(-1); break;
		case axis::neg_y:	y = D(-1); break;
		case axis::neg_z:	z = D(-1); break;
		default:	break;
		}
	}
	/// Get axis value
	constexpr D at(axis a) const {
		...
		case axis::neg_x:	return -x;
		case axis::neg_y:	return -y;
		case axis::neg_z:	return -z;
		default:	break;
		}
	}
};

template<typename D2> inline vector3t<D2> operator* (D2 scalar, const vector3t<D2>& v) { return v * scalar; }
