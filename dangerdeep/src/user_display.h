...
class user_display : public input_event_handler
{
  public:
    ...
    class elem2D
    {
      public:
        ...
        /// Is Mouse over element? Does not check for rotation, just uses 2D
        /// area.
        ...
        auto get_value_range() const
        {
            return std::make_pair(start_value, end_value);
        }
        /// Set the value defined by mouse position @return new set value if
        /// valid
        std::optional<double> set_value(const vector2i& mpos) const;
        /// Set the value as unsigned defined by mouse position @return new set
        /// value if valid
      ...
      protected:
        ...
        vector2i
            center;    ///< Center of the element on screen (used for rotation)
        ...
        double start_value{
            0.0};        ///< For rotating elements the value at start angle
        angle end_angle; ///< For rotating elements the end angle
        double end_value{
            360.0};            ///< For rotating elements the value at end angle
        angle rotation_offset; ///< The offset to use for display
        mutable unsigned phase{0};  ///< The subimage (phase) to use, mutable
                                    ///< because called by const display()
        ...
        double get_angle_range()
            const; ///< Compute range of angles between start and end
    };
    ...
    /// overload with code that deinits data for that display, like freeing
    /// images
    virtual void leave();
  ...
  protected:
    ...
    std::unordered_map<unsigned, unsigned>
        id_to_element; ///< Mapping IDs from definition to elements
    const elem2D&
    element_for_id(unsigned id) const;   ///< Deliver element for a given ID
