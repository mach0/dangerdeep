...
<damageable-parts>
<part id="rudder" x0="" y0="" z0="" x1="" y1="" z1="" weakness="0.2"
repairtime="3600" surfaced="true" repairable="true"/>


parts can be inside the hull and thus damaged or be part of the hull and thus
flooded. some parts are exterior and are not flooded (screws, rudder). the more
are part is damaged the more it is flooded. the game will read the ship
description xml file and a part layout file (3ds). it can determine the total
volume of all parts from the 3ds file. it must know the ships water draught and
weight. formula, if the ship swims: draught * 1 >= weight ? draught is given by
total volume of all parts and height of waterline. when a part is damaged, it
begins to flood with water, at most to the height of the waterline. the higher
the damage, the more water per second gets into it. the ship gets heavier: ship
weight = base weight + flooded_water in cubic meters * 1 if ship weight >
draught then the ship sinks with some speed that depends on the difference
between weight and draught.
When it sinks, the draught increases, and later also the amount of flooded
water. So it can happen, that the ship will not sink totally, but just lies
deeper in the water. if the waterline is too height (<1m below deck?) it will
sink (for ships!) because the water will run through the bulkheads from top, or
it capsizes.
...
B = draught = sum of all parts below waterline, depends on height (pos.z) of
ship. if A > B then ship sinks, i.e. set acceleration.z to -G if B < A set acc.z
to +G, else to 0.

some parts could be mostly above the waterline and not add to the draught, but
may sink below it when the ship gets heavier.
...
a part leaks more if the damage is heavier. wrecked parts will flood fastest and
can not be repaired. the time needed to pump a part dry depends on its volume
and the amount of water the pumps can handle.

damage statii: none, light, medium, heavy, wrecked. (0,25,50,75,100%)
a part can have two or five statii (binary / variable).

we must know how many pumps a ship has and how much water they can pump outside
per second. submarine's hull can be damaged independent on amount of flooded
water... difficult also flooded parts can lead to capsizing, or the ship sinks
because the bow or stern dives below the waterline although the ship would swim
in total.
*/
...
submarine::tank::tank(xml_elem e) :
    type(ballast), volume(e.attrf("volume")), fillrate(e.attrf("fillrate")),
    pos(e.attrf("posx"), e.attrf("posy"), e.attrf("posz")), fill(0),
...
submarine::submarine(game& gm_, const xml_elem& parent) :
    ship(gm_, parent), max_depth(0), dive_to(0), permanent_dive(false),
    ...
    scope_raise_level(0.0f), scope_raise_to_level(0.0f), electric_engine(false),
    ...
    battery_level(0)
{
    ...
    // set all common damageable parts to "no damage", fixme move to ship?,
    // replace by damage editor data reading
...
void submarine::save(xml_elem& parent) const
{
    ...
    // for (vector<sea_object::part>::const_iterator it = parts.begin(); it !=
    // parts.end(); ++it) { save a part node for each entry it->save(out);
    //}
...
void submarine::transfer_torpedo(unsigned from, unsigned to)
{
        ...
        && torpedoes[to].status == stored_torpedo::st_empty)
    {
        ...
        torpedoes[from].remaining_time = torpedoes[to].remaining_time =
            get_torp_transfer_time(
                from, to); // fixme: add time for torpedos already in transfer
                           // (one transfer may block another!)
    }
}
...
void submarine::simulate(double delta_time, game& gm)
{
    ...
    {
        scope_raise_level = std::min(
            scope_raise_to_level,
            scope_raise_level + float(delta_time * scope_move_speed));
    }
    else if (scope_raise_level > scope_raise_to_level)
    {
        scope_raise_level = std::max(
            scope_raise_to_level,
            scope_raise_level - float(delta_time * scope_move_speed));
    }
    ...
    if (gm.is_valid(target))
    {
        ...
        {
            TDC.set_bearing(
                angle(gm.get_object(target).get_pos().xy() - get_pos().xy()));
        }
    }
    ...
    for (unsigned i = 0; i < torpedoes.size(); ++i)
    {
        ...
        if (st.status == stored_torpedo::st_reloading
            || st.status == stored_torpedo::st_unloading)
        { // reloading/unloading
    ...
    if (false /*true*/ /*automatic_reloading*/)
    {
        ...
        for (unsigned i = bow_tube_indices.first; i < bow_tube_indices.second;
             ++i)
        {
        ...
        for (unsigned i = stern_tube_indices.first;
             i < stern_tube_indices.second;
             ++i)
        {
            if (torpedoes[i].status == 0)
            {
...
void submarine::set_target(sea_object_id s, game& gm)
{
    ...
    if (TDC.auto_mode_enabled())
    {
        ...
        TDC.set_target_distance(
            mytarget.get_pos().xy().distance(get_pos().xy()));
...
void submarine::init_fill_torpedo_tubes(const date& d)
{
    ...
    // standard1, special1 2/3, standard2, special2 1/3, 1/2 standard, 1/2
    // special hence random 0-5, 0,1:std1, 2:std2, 3,4:spc1, 5:spc2
    string standard1, standard2, special1, special2, stern, deck;
    // the following if's and definitions are coded after the data from the
    // torpedo xml's. we could also read all xml's, parse them, and create
    // torpedoes accordingly, but it was faster to do it hardcoded, and that
    // data is historic and thus does not change. totally available: TI -
    // standard, fat1 since dec. 42, lut1 since feb. 44, lut2 since june 44 TII
...
double submarine::get_torp_transfer_time(unsigned from, unsigned to) const
{
    ...
    for (unsigned i = flin; i < tlin; ++i)
    {
        ...
        {
            case 1:
                tm += get_bow_reload_time();
                break;
            case 2:
                tm += get_bow_deck_reload_time();
                break;
            case 3:
                tm += get_bow_stern_deck_transfer_time();
                break;
            case 4:
                tm += get_stern_deck_reload_time();
                break;
            case 5:
                tm += get_stern_reload_time();
                break;
        }
    }
    return tm;
...
float submarine::surface_visibility(const vector2& watcher) const
{
    ...
    // above the water. In reality it is hidden in the waves when watched from a
    // longer distance (it doesn't need to be under water, just hidden by higher
    // waves nearby). So the only visible thing of a sub is the conning tower,
    ...
    {
        dive_factor =
            0.1f * (10.0f - depth) * ship::surface_visibility(watcher);
    }
    ...
    if (depth >= 10.0f && depth <= periscope_depth)
    {
        ...
        if (is_scope_up())
        {
            // The visibility of the periscope also depends on the speed its
            // moves through the water. A fast moving periscope with water
            // splashed is much farther visible than a still standing one.
        ...
        dive_factor +=
            diverse_modifiers * (0.5f + 0.5f * speed / max_speed_forward);
    }
...
void submarine::depth_steering_logic()
{
    ...
    */
    ...
    double error1 =
        (bow_depth_rudder.max_angle / bow_depth_rudder.max_turn_speed
         + stern_depth_rudder.max_angle / stern_depth_rudder.max_turn_speed)
        * local_velocity.z * 1.0;
    double error2 =
        (bow_depth_rudder.angle / bow_depth_rudder.max_turn_speed
         + stern_depth_rudder.angle / stern_depth_rudder.max_turn_speed)
...
void submarine::ballast_tank_control_logic(double delta_time)
{
    ...
    // ---------------------------------------
    // const double kg_per_sec = 1000; // 100 liters per second can be
    // flooded/blowed out
    // neutral code: flood as much water into the tanks so the sub floats
    // should be done only when not surfaced, though...
    double s1 = mass + mass_flooded_tanks
                - mymodel->get_base_mesh().volume
                      * 1000.0 /*water_density*/; //, s2 = 0;
    ...
    if (err > 0)
    {
        ...
        /*double rest = */ push_air_to_ballast_tanks(
            1 /*m^3 of air*/ * delta_time);
    ...
    // log_debug("volume="<<mymodel->get_base_mesh().volume<<" mass="<<mass<<"
    // flooded tanks mass="<<mass_flooded_tanks<<" err="<<err);
...
void submarine::depth_charge_explosion(const class depth_charge& dc)
{
    ...
    double damage_radius =
        DAMAGE_DC_RADIUS_SURFACE + get_pos().z * DAMAGE_DC_RADIUS_200M / 200;
    double deadly_radius =
        DEADLY_DC_RADIUS_SURFACE + get_pos().z * DEADLY_DC_RADIUS_200M / 200;
    ...
    double expfac =
        4.605170186 / (damage_radius - deadly_radius); // -4.6 = ln(0.01)

    // fixme: check if relative position is translated correctly to sub pos,
    // e.g. z coordinates seemed to be reversed, a dc exploding below the sub
    // damages the aa gun more than the hull (2003/07/31)
    ...
    log_debug(
        "depth charge explosion distance to sub: " << relpos.length() << "m.");
    ...
    else if (sdlen <= damage_radius)
    { // handle damages
        // add damage
        vector3f bb = mymodel->get_boundbox_size();

        // project relative position to circle on 2d plane (y,z) parallel to
        // sub. circle's radius is proportional to strength. all parts within
        // this circle are affected to damage relative to their distance to the
        // circles center.
        ...
        for (unsigned i = 0; i < parts.size() /*nr_of_parts*/; ++i)
        {
            ...
            log_debug(
                "DC caused damage! relpos " << relpos.x << "," << relpos.y
                                            << "," << relpos.z << " dmg "
                                            << strength);
        }
    }
...
bool submarine::launch_torpedo(int tubenr, const vector3& targetpos, game& gm)
{
    ...
    { // check if target is behind
        ...
        pair<unsigned, unsigned> idx =
            usebowtubes ? get_bow_tube_indices() : get_stern_tube_indices();
        for (unsigned i = idx.first; i < idx.second; ++i)
        {
            if (torpedoes[i].status == stored_torpedo::st_loaded)
            {
    ...
    if (TDC.solution_valid())
    {
        ...
        // cout << "fired at " << fired_at_angle.value() << ", head to " <<
        // torp_head_to.value() << ", is cw nearer " <<
        // torp_head_to.is_clockwise_nearer(fired_at_angle) << "\n";
        xml_doc doc(data_file().get_filename(torpedoes[tubenr].specfilename));
        doc.load();
        torpedo torp(gm, doc.first_child(), torpedoes[tubenr].setup);
        torp.head_to_course(
            torp_head_to,
            fired_at_angle.is_clockwise_nearer(torp_head_to) ? 1 : -1);
        // just hand the torpedo object over to class game. tube is empty after
        // that...
        vector3 torppos =
            position
            + (fired_at_angle.direction() * (get_length() / 2 + 5 /*5m extra*/))
                  .xy0();
...
void submarine::compute_force_and_torque(vector3& F, vector3& T, game& gm) const
{
    ...
    vector3 Fdr, Tdr;
    bow_depth_rudder.compute_force_and_torque(
        Fdr, Tdr, get_local_velocity(), water_density);
    double flowforce = get_throttle_accel() * mass * rudder.deflect_factor();
    double finalflowforce = stern_depth_rudder.compute_force_and_torque(
    ...
    // for forward movement of submarine. So subtract from forward force what
    // does not bypass the rudder.
    ...
    {
        double grav_force =
            it.get_fill() * 1000.0 /* water_density */ * -constant::GRAVITY;
