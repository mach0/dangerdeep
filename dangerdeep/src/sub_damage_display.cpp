    ...
    bool atbottom)
{
    ...
    font_vtremington12->print_wrapped(
        posx + 8, posy + 45, 256 - 16, 20, text, color(0, 0, 128));
}
} // namespace
...
void sub_damage_display::display() const
{
    ...
    for (unsigned i = 0; i < parts.size(); ++i)
    {
        ...
        if (mouse_position.x >= r.x() && mouse_position.x <= r.x() + r.w()
            && mouse_position.y >= r.y() && mouse_position.y <= r.y() + r.h())
        {
            // it is important, that texts are in correct order starting with
            // 400.
            ...
            if (damcat > 0)
            {
                if (mysub->damage_schemes[i].repairable)
                {
                    ...
                    {
                        auto minutes =
                            unsigned(round(parts[i].repairtime / 60.0));
                        dmgstr << texts::get(167) << "\n"
                               << texts::get(170) << minutes
                               << texts::get(minutes == 1 ? 171 : 172);
                    }
                }
                else
                {
