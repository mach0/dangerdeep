...
#endif
        uint8_t high_bits =
            ((bitmask[high_bits_len] << (len - high_bits_len)) & bits)
            >> (len - high_bits_len);
...
ibitstream::ibitstream(std::istream* is, long bufsize) :
...
uint8_t ibitstream::read_byte(uint8_t len)
{
    ...
    if (len > (8 - bit_pos))
    {
        ...
        uint8_t high_bits = (buffer[byte_pos] & bitmask[high_bits_len - 1])
                            << low_bits_len;
        uint8_t low_bits = (buffer[byte_pos + 1]
                            & (bitmask[low_bits_len - 1] << (8 - low_bits_len)))
                           >> (8 - low_bits_len);
