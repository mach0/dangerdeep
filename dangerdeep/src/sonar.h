...
struct noise
{
    ...
    // factor for sea state dependant noise, values here are for max. wave
    // heights, in dB
    ...
    // factor for wave interference in shallow water ( < 250m, 125m in
    // Mediterr.), in dB
    ...
    // additional extra noise constant for cavitation, when running at
    // full/flank speed, in dB
    ...
    noise operator-(const noise& other) const
    {
        ...
        for (unsigned i = 0; i < NR_OF_FREQUENCY_BANDS; ++i)
            result.frequencies[i] =
                std::max(frequencies[i] - other.frequencies[i], 1e-10);
        return result;
    }
...
struct noise_signature
{
    ...
    static const double typical_noise_signature[NR_OF_SHIP_CLASSES]
                                               [noise::NR_OF_FREQUENCY_BANDS];

    struct band_noise_data
    {
        double basic_noise_level; // in dB    try some values, maybe 10
        double speed_factor;      // in dB    0.541 per m/s   (in theory per
                                  // throttle, not speed...)
    };
