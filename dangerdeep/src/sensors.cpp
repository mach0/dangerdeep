...
auto sensor::get_distance_factor(double d) const -> double
{
    double df = 0;
...
auto sensor::is_within_detection_cone(const vector2& r, const angle& h) const
    -> bool
{
...
auto lookout_sensor::is_detected(
    const game* gm,
    const sea_object* d,
    const sea_object* t) const -> bool
{
    bool detected        = false;
    ...
    if (dist < max_view_dist)
    {
        ...
        // A destroyer (771m�) is then visible from its broadside at 15420
        // meters. A corvette (232m�) is then visible from its broadside at 4640
        // meters. A carrier (1087m�) is then visible from its broadside at
        // 21740 meters. A small tanker (576m�) is then visible from its
        // broadside at 11520 meters. A large freighter (889m�) is then visible
        // from its broadside at 17780 meters. A VIIc sub (120m�) is then
        // visible from its broadside at 2400 meters. The factor is obviously
        // too large. A sub can be seen from its broadside at superb conditions
        // in 5km at least, but that would lead to a factor so that large
        // freighters are visible from 37km (~20sm)! Effects like smoke or wake
        // are ignored here, but are essential, fixme!!! A ship's/sub's speed
        // influenced the visibility, especially for subs! fixme: earth
        // curvature is ignored here!!! fixme: we should visualize the
        // visibility for testing purposes.
        const double visfactor = 0.05;

        // this model ignores special features of visibility for water splashes,
        // particles or grenades... all of these have a cross section of 100
        // square meters hard coded for testing, except particles, which have a
        // real cross section

        // multiply with overall visibility factor: max_view_dist/30km.
        // the idea behind this formula is that at night smaller objects are
        // harder to detect. however it's results are bad.
...
auto passive_sonar_sensor::is_detected(
    double& sound_level,
    const game* gm,
    const sea_object* d,
    const sea_object* t) const -> bool
...
auto active_sensor::get_distance_factor(double d) const -> double
{
    double df = 0;
    ...
    const sea_object* t) const -> bool
{
    ...
    if ((dsub && !dsub->is_submerged()) || !dsub)
    {
        ...
        if (is_within_detection_cone(r, d->get_heading()))
        {
            ...
            // Radars use the surface visibility factor. 2004/05/16 fixme adapt
            // constants
...
active_sonar_sensor::active_sonar_sensor(active_sonar_type type) :
    active_sensor()
{
    init(type);
}
...
auto active_sonar_sensor::is_detected(
    const game* gm,
    const sea_object* d,
    const sea_object* t) const -> bool
{
    ...
    else
    {
        ...
        if (tsub && tsub->is_submerged())
        {
            ...
            if (is_within_detection_cone(r, d->get_heading()))
            {
                ...
                double prod =
                    dist_factor * sonar_vis * dnoisefac * depth_factor;
