...
moon::moon()
{
    map_diffuse = std::make_unique<texture>(
        get_texture_dir() + "moon_d.png", texture::LINEAR);
    ...
    map_normal = std::make_unique<texture>(
        mnp, mns, mns, GL_RGB, texture::LINEAR, texture::CLAMP);
    glsl_moon = std::make_unique<glsl_shader_setup>(
        get_shader_dir() + "moon.vshader", get_shader_dir() + "moon.fshader");
    glsl_moon->use();
