...
{
	template<typename T> bool compute(const vector3t<T>& va0,
			    ...
			    const T eps = T(1e-5))
	{
		...
		if (helper::is_zero_with_tolerance(detAq0, eps) && helper::is_zero_with_tolerance(detAq1, eps)) {
		...
		bool b0_legal = (b0 > nullT) && (b0 < detAq0*detAq0) && !helper::is_zero_with_tolerance(detAq0, eps);
		bool b1_legal = (b1 > nullT) && (b1 < detAq1*detAq1) && !helper::is_zero_with_tolerance(detAq1, eps);
		...
		T dx2 = d.x * d.x;
		T dy2 = d.y * d.y;
		bool delta0_legal = !helper::is_zero_with_tolerance(d.y, eps) && (delta0 > nullT) && (delta0 < dy2);
		bool delta1_legal = !helper::is_zero_with_tolerance(d.x, eps) && (delta1 > nullT) && (delta1 < dx2);
