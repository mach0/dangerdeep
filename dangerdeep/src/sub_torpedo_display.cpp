    ...
    const submarine::stored_torpedo& st) const
{
    if (usebow)
    {
        ...
        { // reloading
            element_for_id(element_by_spec(st.specfilename))
                .get_texture()
                .draw(pos);
...
std::vector<vector2i> sub_torpedo_display::get_tubecoords(submarine* sub) const
{
    ...
    // Note that these coordinates should be defined as clickable areas in the
    // layout.xml dump that out to a file for all possible tubes to have the
    // data.
    ...
    const std::vector<vector2i>& tubecoords) const
{
    ...
    {
        if (mouse_position.x >= tubecoords[i].x
            && mouse_position.x < tubecoords[i].x + 128
            && mouse_position.y >= tubecoords[i].y
            && mouse_position.y < tubecoords[i].y + 16)
        {
            return i;
        }
    }
...
sub_torpedo_display::sub_torpedo_display(user_interface& ui_) :
    user_display(ui_, "sub_torpedo"), torptranssrc(ILLEGAL_TUBE),
    desc_texts(get_data_dir()), torp_desc_line(0),
    notepadsheet(texturecache(), "notepadsheet.png")
{
    ...
    element_for_id(et_subtopsideview)
        .set_filename(
            get_data_dir() + data_file().get_rel_path(pl->get_specfilename())
...
void sub_torpedo_display::display() const
{
    ...
    if (torptranssrc != ILLEGAL_TUBE
        && torpedoes[torptranssrc].status
               == submarine::stored_torpedo::st_loaded)
    {
        auto& elem = element_for_id(
            element_by_spec(torpedoes[torptranssrc].specfilename));
    ...
    if (tb != ILLEGAL_TUBE)
    {
        ...
        if (torpedoes[tb].status == submarine::stored_torpedo::st_loaded)
        {
            ...
            try
            {
                torpdesctext = desc_texts.ref(
                    data_file().get_rel_path(sfn) + sfn + "_"
            ...
            catch (error& e)
            {
                ...
                torpdesctext = desc_texts.ref(
                    data_file().get_rel_path(sfn) + sfn + "_en.txt");
            }
            ...
            || torpedoes[tb].status == submarine::stored_torpedo::st_unloading)
        {
            ...
            seconds = torpedoes[tb].remaining_time - floor(hours) * 3600
                      - floor(minutes) * 60;
        ...
        if (left_mouse_button_pressed)
        {
            ...
            if (torpedoes[tb].status == submarine::stored_torpedo::st_reloading
                || torpedoes[tb].status
                       == submarine::stored_torpedo::st_unloading)
            {
    ...
    {
        font_vtremington12->print(
            400, 85, helper::str(sub->num_shells_remaining()), color(0, 0, 0));
    }
