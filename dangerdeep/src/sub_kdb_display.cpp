    ...
    -> bool
{
    ...
    if (m.down())
    {
        ...
        else if (element_for_id(et_turn_wheel)
    ...
    -> std::pair<angle, double>
{
    ...
    for (double ang_scanned = 0; ang_scanned < maxstep;
...
void sub_kdb_display::display() const
{
    ...
    element_for_id(et_pointer)
        .set_value(element_for_id(et_turn_wheel)
                       .get_value()); // fixme get from player's device
